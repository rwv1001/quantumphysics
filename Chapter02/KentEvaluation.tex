...
\begin{figure}[ht!]
	...
	\draw[->,black, thick] [domain=\at/2:\rrange, samples=150]   plot (\x, {wbr(\x)})   ;
	 ...
	 \node[scale=\textscale]  at (2.5,4.1) {$S_{n,f}$}; 
	  \node[scale=\textscale]  at (3.9,4.1) {$S_{n,i}$}; 
	  	  \node[scale=\textscale]  at (5.65,4.1) {$S_{n,0}$}; 
	
	\draw[<->] (-\lrange, \h) node[left, scale=\textscale] {$S_0$} -- (\rrange, \h) node[right, scale=\textscale] {$S_0$};
	\draw[<->] (-\lrange, \a) node[left, scale=\textscale] {$S$} -- (\rrange, \a) node[right, scale=\textscale] {$S$};
				  
	\draw[->, shorten <= 5pt,  shorten >= 1pt] (\psione,\h)  node[above left, scale=\textscale]{$z_0$}-- (\psione,\a) ;
	...
	\draw (\psione,\wttestx) node[ scoresquare, fill=gray]  {} node [black,below=6,right,scale=\textscale] {$t_0$};
	...
	\draw [black,fill](\xendz,\a) circle [radius=\circsize] node [black,above,scale=\textscale] {$\gamma^{(i)}$}; 
	\draw [black,fill](\qxendz,\a) circle [radius=\circsize] node [black,above,scale=\textscale] {$\gamma^{(f)}$}; 
	%\draw [black,fill] (\xendz,\a) circle [radius=\circsize] node [black,above,scale=\textscale] {$\gamma_1$}; 
	\end{tikzpicture}% pic 1
	
	\vspace*{2px}
	\caption{Depicts an experiment where the state of some photons $\gamma^{(i)}$ on the hypersurface $S$ determines the initial conditions of an experimental setup at spacetime location $(z_1, t_i)$, and the state of photons $\gamma^{(f)}$ on the hypersurface $S$ determines the measurement outcome of the experiment at a time shortly after $t_f$, and it is assumed that no incoming photons have interacted with the experiment after the $\gamma^{(i)}$ photons and before the $\gamma^{(f)}$ photons have interacted with the experiment.  }
	\label{pisolution}
	\end{figure}
