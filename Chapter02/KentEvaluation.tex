...
A final question to consider in this chapter is the nature of Kent's beables. As discussed on page \pageref{beabledef}, Bell hoped for a more satisfactory theory than standard quantum theory. This more satisfactory theory was to be a theory of beables rather than a theory of observables, and these beables would form the underlying reality that gives rise to all the familiar things in the world around us. Now in the context of Kent's interpretation, the determinate mass/energy density $\tau_S(x)$ at a location $x\in S$ seems like a good candidate to be a beable. But what is less obvious is what the beables should be for a spacetime locate $y$ prior to $S$. Kent seems to be saying that the conditional expectation value  $\ev*{\hat{T}^{\mu\nu}(y)}_{\tau_S}$ is a beable at $y$. Now in some situations, the determinate values of $\tau_S(x)$ on $S$ will give rise to a determinate value for $T^{\mu\nu}(y)$, in which case the conditional expectation $\ev*{\hat{T}^{\mu\nu}(y)}_{\tau_S}$ will be identical to the determinate value for $T^{\mu\nu}(y)$. But there will inevitably be situations in which the determinate values of $\tau_S(x)$ on $S$ will not give rise to a determinate value of $T^{\mu\nu}(y)$. In the notion defined in section \ref{LorentzInvarianceSection} on page \pageref{tauprojection}, we may find that the state $\pi_n\ket{\Psi_n}$ at $y$ is in a superposition of eigenstates of $\hat{T}^{\mu,\nu}(y)$. But in that case, it seems very dubious to claim that the expectation value $\ev*{\hat{T}^{\mu\nu}(y)}_{\tau_S}$ is the true state of reality and hence the beable at $y$. As an analogy, it seems a bit like saying the throw of a six-sided dice could yield a beable of 3.5 for its outcome since 3.5 is the expectation value for the throw of a dice.
Imposing this degree of definiteness at $y$ seems unnatural and unnecessary. 

To my mind, it seems more natural to suppose that there are limitations to the definiteness that we can attribute to physical reality. So in the context of Kent's interpretation, the mass/energy density $\tau_S(x)$ on $S$ would be definite, and some definite facts about physical reality would flow from the definite facts about $S$. But there would also be much indefiniteness about physical reality. This occurs whenever the information in $\tau_S(x)$ is insufficient to determine whether the state $\pi_n\ket{\Psi_n}$ is an eigenstate of some observable. If light had interacted with the location $y$ in a different way, then this might be able to settle which eigenstate $\pi_n\ket{\Psi_n}$ was in, but this definite outcome would then result in $y$ being indefinite with respect to other observables. But even though there will inevitably be many physical quantities of a system that lack definiteness, we can still make claims about how mass/energy could have been measured on $S$ and hence how likely a physical quantity could have had a particular value corresponding to this possible measurement on $S$.

Another issue that should concern us is the question of what should be the  subject of the beables. Any beable presupposes a subject as well as a property that belongs to the subject. So in the case of Kent's beables, the spacetime location $y$ would be the subject and $\ev*{\hat{T}^{\mu\nu}(y)}_{\tau_S}$ would be a property of the subject. However, in section \ref{kentpi}, we found that in order to deal with joint probabilities in Bell-type experiments, it was necessary to calculate expectation values for observables that depended on two spacetime locations rather than just one spacetime location. Now perhaps some people would argue that joint probabilities are not beables and therefore conclude that the need to use multiple spacetime locations in determining joint probabilities is irrelevant when it comes to deciding what the subject of a beable should be. But in defense of their relevance, it does seem that joint probabilities are saying something about reality, and in the absence of hidden variables, these joint probabilities can't be reduced to some more basic reality of which the components would only depend on  single spacetime locations. So I think this is a reason to attribute beability to probabilities. 

But note that once we allow for beables whose subject consists in multiple locations, can help us to avoid some strange consequences that occur if we just insist on single locations the subject of a beable. For consider figure \ref{strangesubject}.
...
with $\abs{a}^2+\abs{b}^2=1$ (and where we have omitted all other terms corresponding to other spacetime locations). A photon then interacts with the joint system and is ``measured'' on $S$ to be in state $\ket{\gamma}$ thus determining the mass of the composite system to be located at $z_L$. Now we consider the observable $[\psi]_L$ and suppose its corresponding $z_L$-beable has as its subject a single spacetime location $(t,z_L)$, so that the expectation of $[\psi]_L$ is conditioned on $S_n\cap S$ as depicted in figure \ref{strangesubject} (a). Then as soon as the photon has interacted with the mass centered at $z_L$, we can say that the system has collapsed to this mass center. But if the photon registers at a location on $S$ that is within the light cone of $(t, z_R)$, and we consider the observable $[\psi]_R$ and its corresponding $z_R$-beable, then there is nothing to condition on in $S$ when calculating the expectation value of $[\psi]_R$, and so we will obtain a non-zero answer for this $z_R$-beable, even though according to the $z_L$-beable all the mass is located at $z_L$. This seems very strange. 

If however, we consider the beable whose subject consists of both $(t, z_L)$ and $(t, z_R)$, then because we have to choose a hypersurface that contains both $(t, z_L)$ and $(t, z_R)$ as depicted in figure \ref{strangesubject} (b), the system won't collapse to the mass center at $z_L$ as soon as the photon interacts with it. Rather, it will only collapse to this mass center once the photon registered on $S$ is outside the light cones of  both $(t, z_L)$ and $(t, z_R)$. Once this time has occurred, the $z_L$ and the $z_R$ will no longer be entangled, and so we would be able to consider beables whose subjects were the single locations at $z_L$ and $z_R$.

This suggests we should take all the spacetime locations in an entangled system to be the subject of a beable. Particle interactions and particle spreading will tend to induce entanglement between spacetime locations, but ``measurement'' on $S$ will tend to disentangle spacetime locations. For example, if we have an entangled state of the form
...
There is the question of which hypersurface we should choose to contain all the spacetime locations that form the subject of a beable, for we need to be able to do this in a natural way lest we sacrifice Lorentz invariance. An obvious choice for such a hypersurface would be one in which all the entangled spacetime locations are simultaneous and in which the expectation value of the center of mass of the system had zero velocity. Then we could the corresponding beables of the entangled system to be Lorentz invariant in  a way analogous to the rest mass of a particle being Lorentz invariant.

Having specified what the subject of a beable should be, we can finally consider what the beable itself should be. I propose we take the reality signified by the reduced density matrix (conditioned on the available information in $S$) of an entangled system to be the beable. In section \ref{Kentdecoherencesection}, I showed how to calculate these reduced density matrices when the system was a single location, but there is nothing to stop us from calculating the reduced density matrices for systems of entangled spacetime locations. I am attracted to this proposal since because it allows for definite outcomes as in the case when the reduced density matrix is a pure state such as in equation (\ref{purerho}). But the reduced density matrix also captures the reality of possibility rather than forcing on us an overly rigid definiteness to the whole of reality. The meaning of this possibility is grounded in the real possibilities that the ``measurement'' on $S$ could have been other than it is.
