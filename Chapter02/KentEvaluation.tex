...
Also note that we can typically expect the $\ket{\gamma_i^{\mathcal{S}}}$ state to be independent of the $\ket{\gamma_i^{\mathcal{A}}}$. Therefore, since $\ket{\gamma_i^{\mathcal{A}}}$ will determine the measurement choice, and since $\ket{\gamma_i^{\mathcal{S}}}$ determines the initial state of the particle, we can expect the state of the particle to be independent of the measurement choice in Kent's interpretation, and thus fulfil one of the necessary criteria (i.e. criterion \ref{hidden2}) for PI to be a well-defined notion.

\subsection{Kent's interpretation and Parameter Independence}
...
\draw (\psione,\wttestx) node[ scoresquare, fill=gray]  {} node [black,below=6,right,scale=\textscale] {$t_m$};
	...
	\draw [black,fill](\qxendz,\a) circle [radius=\circsize] node [black,above=9,right=-4,scale=\textscale] {$\gamma_m^{(\mathcal{A}_L)}$}; 
	\draw [black,fill](-\qxendz,\a) circle [radius=\circsize] node [black,above=9,right=-4,scale=\textscale] {$\gamma_m^{(\mathcal{A}_R)}$}; 
	
	%\draw [black,fill] (\xendz,\a) circle [radius=\circsize] node [black,above,scale=\textscale] {$\gamma_L$}; 
	\end{tikzpicture}% pic 1
	
	\vspace*{2px}
	\caption{Depicts an a Bell-type experiment where the state of some photons $\gamma_i^{(\mathcal{A}_L)}$ and $\gamma_i^{(\mathcal{A}_R)}$ on the hypersurface $S$ determines the choice of measurement parameters of the left wing and right wing of the experiment respectively, and some photons $\gamma_m^{(\mathcal{A}_L)}$ and $\gamma_m^{(\mathcal{A}_R)}$ on the hypersurface $S$ determine the measurement outcome of the experiment on the left wing and the right wing respectively. The dashed lines on the hypersurfaces $S_{n,m}$ and $S_{n,i}$ indicate other choices of for the hypersurfaces, but they still lead to the same probability being calculated.   }
	\label{bellsolution}
	\end{figure}
...
\end{equation}
We saw  in footnote \ref{bellstate2pf} on \pageref{bellstate2pf} that a Bell state does not depend on the orientation of $\uvb{s}$, so without loss of generality, we can suppose that the $\ket*{\uvbp{s}}_L$ and $\ket*{\uvbm{s}}_L$ are pointer states for the apparatus on the left-wing of the experiment. This means there will be a ready state $\ket{a}_L$  as well as two states $\ket{a+}_L$ and $\ket{a-}_L$ of the left wing apparatus such that 
$$\ket*{\uvbpm{s}}_L\ket{a_r}_L\rightarrow\ket*{\uvbpm{s}}_L\ket{a\pm}_L.$$

As for the right wing of the experiment, we let  $\ket*{\bm{\hat{s}_\theta+}}_R$ and $\ket*{\bm{\hat{s}_\theta-}}_R$ be pointer states for the apparatus so that there is a ready state $\ket{a_\theta}_R$  as well as two states $\ket{a+_\theta}_R$ and $\ket{a-_\theta}_L$ of the right wing apparatus such that 
$$\ket*{\uvbpm{s}}_R\ket{a_r}_R\rightarrow\ket*{\uvbpm{s}}_R\ket{a\pm}_R.$$
...
Using the Born rule, we therefore see that given a measurement of $T_S(x)$ determines the state of the hypersurface $S_{n,i}$ to be in state $\ket{\Phi_{n,i}}$, then the probability that the hypersurface $S_{n,m}$ will be found to be in the state 
$$\ket*{\uvbp{s}}_L\ket*{\bm{\hat{s}_\theta+}}_R\ket{a\pm}_L\ket{a_\theta+}_R\ket{\gamma_{m,+}^{\mathcal{A}_L}}\ket{\gamma_{m,+}^{\mathcal{A}_R}}$$
will be 
$$\frac{1}{2}\abs{\beta_\theta}^2=\frac{1}{2}\sin^2(\theta/2) $$
From this it follows that probability that the left wing particle will be in state $\ket*{\uvbp{s}}_L$ and that the right wing particle will be in state $\ket*{\bm{\hat{s}_\theta+}}_R$ given the initial conditions will also be $\frac{1}{2}\sin^2(\theta/2) $. This is the same probability as that given by standard quantum theory on page \pageref{bellsin}. 
...
$[\uvbp{s}]_L [\bm{\hat{s}_\theta+}]_L $, and with the adapted sequence $S_{n,i}$ of hypersurfaces, we can calculate the conditional expectation
$$\ev*{[\bm{\hat{s}_\theta+}]_L[\bm{\hat{s}_\theta+}]_R}_{\tau_S}=\lim_{n\rightarrow\infty}\frac{\ev*{\pi_{n,i}[\bm{\hat{s}_\theta+}]_L[\bm{\hat{s}_\theta+}]_R}{\Psi_{n,i}}}{\ev*{\pi_{n,i}}{\Psi_{n,i}}}.$$
With this adaption and the notional measurement of $T_S(x)$ on $S$ described in this section, it is easy to see that 
$$\ev*{[\bm{\hat{s}_\theta+}]_L[\bm{\hat{s}_\theta+}]_R}_{\tau_S}=\frac{1}{2}\sin^2(\theta/2)$$ 
which is the joint probability for finding the left wing particle in state $\ket{\uvbp{s}}_L$ and the right wing particle in state $\ket{\bm{\hat{s}_\theta+}}_R$. Thus, we can adapt Kent's model so that criterion \ref{hidden4} of page \pageref{hidden4} is satisfied and gives the same probabilities as standard quantum physics. Hence, PI holds in Kent's interpretation.
