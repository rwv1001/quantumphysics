...
$$\ket*{\psi'}=U(\Lambda)\ket*{\psi}.\protect\footnotemark$$\footnotetext{For more details, see \cite{Wigner1939}.}We now wish to describe how observables transform under the action of $U(\Lambda)$. First notice that when I said that $\hat{T}^{\mu\nu}(x)$ is the observable in the Tomonaga-Schwinger picture whose eigenstates with eigenvalues $\tau$ are the states of $S_n$ for which an observer $O$ observes the stress-energy tensor $T^{\mu\nu}(x)$ to take the value $\tau$ at $x$, I made no indication that the Hermitian operator $\hat{T}^{\mu\nu}(x)$ had any dependence on the observer $O$. A different observer will indeed make different observations, but we adopt the convention that these differences are encapsulated in the different states that $O$ and $O'$ deem $S_n$ to be in. 

At this point, it is worth clarifying the slightly different meanings of $T^{\mu\nu}(x)$, $T^{\prime\mu\nu}(x)$, $\tau^{\mu\nu}(x)$, $\tau^{\prime\mu\nu}(x)$, and $\hat{T}^{\mu\nu}(x)$. We use $T^{\mu\nu}(x)$ to refer to the description of the physical quantity that is being observed by $O$. Thus, $T^{\mu\nu}(x)$ is shorthand for the description ``the $\mu\nu$-component of the stress-energy tensor that $O$ observes at the spacetime location belonging to $S$ that $O$ describes as  $x$.'' Similarly,  $T^{\prime\mu\nu}(x)$ is shorthand for the description ``the $\mu\nu$-component of the stress-energy tensor that $O'$ observes at a spacetime location  belonging to $S$ that $O'$ describes as $x'$.'' $\tau^{\mu\nu}(x)$ stands for a particular (real) value of the physical quantity described by $T^\mu\nu(x)$ that $O$ observes, and $\tau^{\prime\mu\nu}(x)$ is defined similarly with respect to observer $O'$. $\hat{T}^{\mu\nu}(x)$ for $x\in S$ is the observable (i.e. Hermitian operator) such that if observer $O$ deemed $S$ to be in an eigenstate $\ket*{\psi}$ of $\hat{T}^{\mu\nu}(x)$ with eigenvalue $\tau$ (a real number), then observer $O$ would observe the physical quantity described by  $T^{\mu\nu}(x)$ to have the value $\tau$,\footnote{Note that we consider a real number $\tau$ here rather than a real valued function $\tau^{\mu\nu}(x)$ of spacetime locations $x\in S$ and indices $\mu$ and $\nu$ since $\hat{T}^{\mu\nu}(x)$ will not in general commute for different values of $\mu$, $\nu$, so we won't be able to find a state which is a simultaneous eigenstate for all the different observables $\hat{T}^{\mu\nu}(x)$, though we may find a state which is very close to being an eigenstate of all the  $\hat{T}^{\mu\nu}(x)$ for different $\mu$ and $\nu$.} and likewise, $\hat{T}^{\mu\nu}(x')$ is the observable such that if observer $O'$ deemed $S$ to be in an eigenstate $\ket*{\psi'}$ of  $\hat{T}^{\mu\nu}(x')$ with eigenvalue $\tau'$, then observer $O'$ would observe the physical quantity described by  $T^{\prime\mu\nu}(x')$ to have the value $\tau'$. 


Having made this point of clarifcation, since we are adopting the convention that the differences between the observations of $O$ and $O'$ are encapsulated in the different states that $O$ and $O'$ deem $S_n$ to be in, it follows that if $\ket*{\psi}\in H_{S_n}$ is a state for which $T^{\mu\nu}(x)\approx \tau^{\mu\nu}(x)$,\footnote{We say approximately ($\approx$) here since the operators $\hat{T}^{\mu\nu}$ will not in general commute, so we won't typically be able to find a state $\ket*{\psi}$ which is an eigenstate for all the observables $\hat{T}^{\mu\nu}$. It is the non-commutativity of observables that is responsible for Heisenberg's uncertainty principle.} and if $\ket*{\psi'}\in H_{S_n}$ is a state for which $T^{\prime\mu\nu}(x')\approx \tau^{\mu\nu}(x')$, then
\begin{subequations}
\begin{align}
...
\end{equation}
Therefore, in order for (\ref{lorentztensor}) to hold for ${T'}^{\mu\nu}(x')$ in the classical limit (where we treat the stress-energy observables as though they commute with each other and replace the approximations by equalities), plugging an operator form of  (\ref{lorentztensor}) into (\ref{stress3}), we must have
...
As for the definition of \textbf{empirical adequacy}\index{empirical adequacy!formal definition} (EA), using the notation of criteria \ref{hidden5} and \ref{hidden4}, this states that
