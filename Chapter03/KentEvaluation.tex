...
$$\ket*{\psi'}=U(\Lambda)\ket*{\psi}.\protect\footnotemark$$\footnotetext{For more details, see \cite{Wigner1939}.}We now wish to describe how observables transform under the action of $U(\Lambda)$. First notice that when I said that $\hat{T}^{\mu\nu}(x)$ is the observable in the Tomonaga-Schwinger picture whose eigenstates with eigenvalues $\tau$ are the states of $S_n$ for which an observer $O$ observes the stress-energy tensor $T^{\mu\nu}(x)$ to take the value $\tau$ at $x$, I made no indication that the Hermitian operator $\hat{T}^{\mu\nu}(x)$ had any dependence on the observer $O$. A different observer will indeed make different observerations, but we adopt the convention that these differences are encapsulated in the different states that $O$ and $O'$ deem $S_n$ to be in. Thus, if $\ket*{\psi}\in H_{S_n}$ is a state for which $O$  observed the stress-energy tensor at $x$ to be (approximately\footnote{We say approximately here since the operators $\hat{T}^{\mu\nu}$ will not in general commute, so we won't typically be able to find a state $\ket*{\psi}$ which is an eigenstate for all the observables $\hat{T}^{\mu\nu}$. It is the non-commutativity of observables that is responsible for Heisenberg's uncertainty principle.}) $T^{\mu\nu}(x),$ and if $\ket*{\psi'}\in H_{S_n}$ is a state for which $O'$ observed the stress-energy tensor at $x'$ to be (approximately) ${T'}^{\mu\nu}(x'),$ then
\begin{subequations}
\begin{align}
...
\begin{equation}\label{stress3}
    U(\Lambda)^{-1}\hat{T}^{\mu\nu}(x')U(\Lambda)\ket{\psi}={T'}^{\mu\nu}(x')\ket{\psi}.
\end{equation}
Therefore, in order for (\ref{lorentztensor}) to hold for ${T'}^{\mu\nu}(x')$ in the classical limit (where we treat the stress-energy observables as though they commute with each other), plugging an operator form of  (\ref{lorentztensor}) into (\ref{stress3}), we must have
