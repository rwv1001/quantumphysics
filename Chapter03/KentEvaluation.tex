...
\end{equation}
where $\ket*{\Xi^{(S^1(y_i))}_i}$ is a simultaneous $\hat{T}_S$-eigenstate over the remainder of $S^1(y_i)$. Note that it is not necessary that the state $\ket*{\gamma^{(\mathcal{S})}_i}$ is caused by photons that have interacted directly with $\mathcal{S}$. Rather, it is sufficient that the photons have been reflected from some system $\mathcal{B}$, say, that has interacted with $\mathcal{S}$ for which $\ket*{s}$ is a pointer state, and that enough photons from $\mathcal{B}$ have intersected $S$ so as to distinguish its different pointer states. 
...
We can also choose a set of $\lambda$ so that criterion \ref{hidden5} holds. To do this, we consider equation (\ref{gammadecomp}) which presupposes there is a subregion of $S^1(y_i)$ which determines the state $\mathcal{S}$ and another non-overlapping subregion that determines the state $\mathcal{A}$. In general, we wouldn't be able to make this association between subregions of $S^1(y_i)$ and states of  $\mathcal{S}$  and $\mathcal{A}$ -- after all, $\mathcal{A}$ might not even exist. But we should be able to make this association if an appropriate choice for the state of the remained of $S^1(y_i)$ is made. In (\ref{gammadecomp}), $\ket*{\Xi_i^{(S^1(y_i))}}$ is able to serve this role. We can then suppose that $\ket*{\gamma_i^{(\mathcal{S})}}$ is from a basis $\Lambda_{\mathcal{S}}=\{\ket*{\gamma_{i,1}^{(\mathcal{S})}},\ket*{\gamma_{i,2}^{(\mathcal{S})}},\ldots\}$ of states that describe all the possible states of $\mathcal{S}$ given  $\ket*{\Xi_i^{(S^1(y_i))}}$ and $\ket*{\Psi_0}$, and likewise, we can suppose that $\ket*{\gamma_i^{(\mathcal{A})}}$ is from a basis $\Lambda_{\mathcal{A}}=\{\ket*{\gamma_{i,1}^{(\mathcal{A})}},\ket*{\gamma_{i,2}^{(\mathcal{A})}},\ldots\}$ of states that describe all the possible states of $\mathcal{A}$ given  $\ket*{\Xi_i^{(S^1(y_i))}}$. We could then take the $\lambda$ of the system $\mathcal{S}$ in criterion to be one of the basis states in $\Lambda_{\mathcal{S}}$. Given that the interpreptation of presupposes $\ket*{\Xi_i^{(S^1(y_i))}}$, we would take the probability $p_\lambda$ for $\lambda = \ket*{\gamma_i^{(\mathcal{S})}}$ to be the probability that notional measurement on $S$ given $\ket*{\Xi_i^{(S^1(y_i))}}$ and $\ket*{\Psi_0}$ corresponded to a simultaneous $\hat{T}_S$-eigenstate having in it the   component $ \ket*{\gamma_i^{(\mathcal{S})}}$ for the subregion of $S^1(y_1)$ that determined the state of $\mathcal{S}$. The formula for this probability would therefore be ***************************************
