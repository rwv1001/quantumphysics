...
\end{equation}
where $\ket*{\Xi^{(S^1(y_i))}_i}$ is a simultaneous $\hat{T}_S$-eigenstate over the remainder of $S^1(y_i)$, i.e. for the subregion $S^1(y_i)\setminus(S_{\mathcal{S}}\cup S_{\mathcal{A}})$. It will be convenient to say that the notional measurement of the mass-energy density on the subregion $S_{\mathcal{S}}$ (for example) is $\ket*{\gamma_i^{\mathcal{S}}}$ when we mean that the mass-energy density measurement on the subregion $\mathcal{S}$ is given by the simultaneous $\hat{T}_S$-eigenvalue of $\ket*{\gamma_i^{\mathcal{S}}}$ on $\mathcal{S}$. Note that it is not necessary that the state $\ket*{\gamma^{(\mathcal{S})}_i}$ is caused by photons that have interacted directly with $\mathcal{S}$. Rather, it is sufficient that the photons have been reflected from some system $\mathcal{B}$, say, that has interacted with $\mathcal{S}$ for which $\ket*{s}$ is a pointer state, and that enough photons from $\mathcal{B}$ have intersected $S$ so as to distinguish its different pointer states. 
...
\begin{equation}
	\begin{split}
U_{S_{n,m},S_{n,i}}\ket*{\uvbpm{s}}_L\ket*{\bm{\hat{s}_\theta\pm'}}_R\ket{a}_L&\ket{a}_R\ket*{\gamma_i^{(\mathcal{A}_L)}}\ket*{\gamma_i^{(\mathcal{A}_R)}}\ket*{\xi_{n,i}}\\
&=\ket*{\uvbpm{s}}_L\ket*{\bm{\hat{s}_\theta\pm'}}_R\ket{a\pm}_L\ket{a_\theta\pm'}_R\ket*{\gamma_{m,\pm}^{(\mathcal{A}_L)}}\ket*{\gamma_{m,\pm'}^{(\mathcal{A}_R)}}\ket*{\xi_{n,m}}
	\end{split}
\end{equation}
where $\ket*{\gamma_{m,\pm}^{(\mathcal{A}_L)}}$ are the states of a subregion of $S_{n,m}$ containing $S_{\mathcal{A}_L}$ that would determine the left wing apparatus to be in the state $\ket{a\pm}_L$, where $\ket*{\gamma_{m,\pm}^{(\mathcal{A}_R)}}$ are the states of a subregion of $S_{n,m}$ containing $S_{\mathcal{A}_R}$ that would determine the right wing apparatus to be in the state $\ket{a_\theta\pm'}_R$, and where  $\ket*{\xi_{n,m}}$ corresponds to the state of the subregion of $S_{n,i}$ not determined by $\ket*{\uvbpm{s}}_L\ket*{\uvbmp{s}}_R$, $\ket{a}_L$, $\ket{a}_R$, $\ket*{\gamma_{m,\pm}^{(\mathcal{A}_L)}}$ or $\ket*{\gamma_{m,\pm}^{(\mathcal{A}_R)}}$.*************
