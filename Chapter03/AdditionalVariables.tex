...
Now Kent supposes that a fictional measurement is made over the whole of $S$ to determine a measurement outcome for $T_S(x)$ which we denote as $\tau_S(x)$ for every $x\in S$. It is these $\tau_S(x)$ that are the additional variables of his theory. 

Clearly,  Kent is not assuming that such a measurement could be performed on $S$ in reality, hence his reason for referring to the measurement as fictional. However, Kent does seem to be saying that there would be a fact of the matter about what value $T_S(x)$ would have at each $x\in S$. So although $S$ is in the far distant future, one can think of the values $\tau_S(x)$ for $T_S(x)$ as being determined primordially, and as a consequence of the primordial determination, physical reality will evolve in a particular way consistent with $T_S(x)$ being $\tau_S(x)$ if a measurement were to be made on $S$. 

Now we saw on page \pageref{eigendef}, that for any measurement on a physical system, we associate and observable whose eigenstates describe a possible state of the physical system in which the measurement is given by the observable's corresponding eigenvalue. Now in the case of the fictional measurement of $T_S(x)$, there will be lots of observables, denoted by $\hat{T}_S(x)$, corresponding to all the different places $x\in S$ where the measurement could be made. Therefore, if a measurement were made on $S$ for all $x\in S$, the corresponding state of $S$ would not just be an eigenstate of one observable $\hat{T}_S(x)$, but rather, it would have to be an eigenstate of every observable $\hat{T}_S(x)$ for every $x\in S$. We will therefore need to speak of simultaneous $\hat{T}_S$-eigenstates and simultaneous $\hat{T}_S$-eigenvalues as described in more detail on page \pageref{simultaneous} in order to specify the quantum state of $S$ on which $T_S(x)$ has a determinate value of $\tau_S(x)$ for every $x\in S$. Given that there is such a quantum state, we can then work out via the Born Rule the probability this state would be selected given the initial state of the universe.
