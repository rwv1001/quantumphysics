...
The recipe for defining Kent's $T^{\mu\nu}(y)$-beable is first to select an outcome $\tau_S$ that is defined over all of $S$. The outcome $\tau_S$ is selected with probability determined by the Born rule using equation (\ref{bornpsi}).\footnote{Kent is a bit vague about what he means for  $\tau_S$ to be selected. He only states that it is randomly selected with a probability given by the Born rule. Presumably he is not understanding probability in a frequentist sense given that he is proposing a one-world interpretation. Also, it hard  to think of this probability in Bayesian terms given that the amount of information needed to specify $\tau_S$ and the initial state $\ket*{\Psi_0}$ of $S_0$ would be unimaginably large, and so it would be impossible for any human being to form posterior beliefs about a particular $\tau_S$ given the state $\ket*{\Psi_0}$. *********************  If there is only one state $\ket*{\Psi}$ such that $\hat{T}_S(x)\ket*{\Psi}=\tau_S(x)\ket*{\Psi}$ for all $x\in S$, then the probability $P(\tau_S)$ that $\tau_S$ is selected will be precisely the probability given by equation (\ref{bornpsi}). But if there are several states $\{\ket*{\Psi_\alpha}:\alpha\}$ such that  $\hat{T}_S(x)\ket*{\Psi_\alpha}=\tau_S(x)\ket*{\Psi_\alpha}$, then the probability $P(\tau_S)$ that $\tau_S$ is selected will be 
...
\begin{equation}\label{qrnProb}P(q(\tau) \, \&\,  r_n) =\sum_{\substack{i \text{ such that } \tau^{(i)}=\tau \\ \text{and }\tau_S^{(i)}(x)=\tau_S(x)\\ \text{ for all }x\in S_n\cap S }}\abs{\mel{\Psi_n^{(i)}}{U_{SS_0}}{\Psi_0}}^2.
\end{equation}
