...
%DIF DEL Alex010723.tex      Wed Aug  2 20:23:26 2023
%DIF ADD revised060823.tex   Sun Aug  6 10:45:59 2023
...
%DIF < 
%DIF -------
%\usepackage[inline]{showlabels} %DIF > 
%DIF < %\makeindex
...
{\centering \DIFdelbegin \DIFdel{The text of this page is brief and generally there is no ending punctuation }\DIFdelend \DIFaddbegin \emph{\DIFadd{To my parents who taught me the most valuable lessons of all}} \DIFaddend \par}
%----------%----------%----------%----------%----------%----------%----------%----------%
%  End Dedication
%----------%----------%----------%----------%----------%----------%----------%----------%
...
%DIF < \makeindex
%DIF > \usepackage[inline]{showlabels}
...
%DIF > \usepackage[inline]{showlabels}
\DIFaddbegin \newpage 
\DIFaddend % flatex input: [Chapter01/chapterIntro.tex]
\begin{comment}
...
\DIFdelbeginFL %DIFDELCMD < \caption[Caption for LOF]{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Stern-Gerlach Experiment]{\DIFaddendFL The Stern-Gerlach Experiment.\protect\footnotemark}
\label{stern}
\end{figure}
\footnotetext{ Original diagram drawn by Theresa Knott. Labeling was modified for use in this dissertation. This image is licensed under the Creative Commons Attribution-Share Alike 4.0 International license. Source: https://commons.wikimedia.org/wiki/File:Stern-Gerlach\_experiment\_svg.svg\DIFaddbegin \DIFadd{.}\DIFaddend }
...
\vspace*{10px}
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Locations of detections on detection screen]{\DIFaddendFL Locations of detections before and after rotating the magnetic field by an angle $\theta$. Before rotation, the particles can be detected at either location $\uvbp{a}$ or location $\uvbm{a}$. After the rotation, particles can be detected at either location $\uvbp{b}$ or $\uvbm{b}$. }
\label{rotate}
\end{figure}
\\ \noindent
...
the real factual situation of the system $\mathcal{S}_2$ should be independent of what is done to the system $\mathcal{S}_1$.\footnote{Einstein expressed this locality principle in his autobiographical notes: ``But on one supposition we should, in my opinion, absolutely hold fast: the real factual situation of the system $\mathcal{S}_2$ is independent of what is done with the system $\mathcal{S}_1$, which is spatially separated from the former.'' \cite[p. 85]{EinsteinLocality}.} \DIFdelbegin \DIFdel{If }\DIFdelend \DIFaddbegin \DIFadd{This principle seemed so intuitive to Einstein, he was convinced it must be true.}\footnote{\DIFadd{\mbox{%DIFAUXCMD
\cite[449]{IsaacsonWalter2008E:hl}}\hskip0pt%DIFAUXCMD
.}} \DIFadd{Writing to Max Born, Einstein famously said ``Physics should represent a reality in time and space, free from spooky  action at a distance.''}\footnote{\DIFadd{From a letter written March 3, 1947, \mbox{%DIFAUXCMD
\cite[155]{BornMax2005TBl:}}\hskip0pt%DIFAUXCMD
. Quoted from \mbox{%DIFAUXCMD
\cite[450]{IsaacsonWalter2008E:hl}}\hskip0pt%DIFAUXCMD
.}} \DIFadd{Although Einstein offered no argument in support of his principle of locality, several arguments have been proposed in its favor. For instance,  Van Laer makes the following argument: 
}\begin{adjustwidth}{1cm}{}
	\begin{displayquote}
\DIFadd{Suppose that body $A$ is located in void space, and that body $P$ is now brought into its vicinity. If $P$ is acted on by $A$, then either $A$'s activity ``across'' the intervening space pre-existed $P$'s arrival, in which case an action existed without an object and without a correlative passion, both of which run counter to the }\emph{\DIFadd{ratio}} \DIFadd{of action; or $A$'s activity only occurred when $P$ became present to it, in which case $A$ would somehow have to have ``known'' $P$'s becoming present, which is not consonant with the nature of mere body as such.}\footnote{\DIFadd{See \mbox{%DIFAUXCMD
\cite[59-144]{LaerP.H.van1953Pp}}\hskip0pt%DIFAUXCMD
. The quote here is from \mbox{%DIFAUXCMD
\cite[246]{Grove}}\hskip0pt%DIFAUXCMD
, who in turn quotes a slightly modified version of Van Laer's argument found in \mbox{%DIFAUXCMD
\cite[257]{TallaricoJames1962AaaD}}\hskip0pt%DIFAUXCMD
. Grove also appeals to the Aristotelian doctrine of hylomorphism in order to defend the principle of locality: 
	}\begin{displayquote}
        \DIFadd{What does it mean for one hylomorphic substance to act on another? It is for the one to effect accidental or (by way of the accidental) substantial change in the other. As the accidents involved are inseparable from spatiality or extension, so must be the operations which occur through them. That is to say, that the interactions of cosmic (nonintellectual) beings are in essence spatial (and therefore spatiotemporal): they involve that which characterizes the spatial as such, i.e., ``parts outside parts,'' extendedness and adjacency. For one hylomorphic being to act on another (and for that other to ``receive'' the action of the one), there must be an ontological ground or commonality which is located precisely in their spatiality; they must ``share space,'' not in violation of the principle of unilocality but along a common boundary, thereby forming a unity through relation, i.e., ``existing with respect to one another.''
    }\end{displayquote}
  }
\end{displayquote}
\end{adjustwidth}

\DIFadd{Now if }\DIFaddend Einstein's locality principle holds, we would be able to attribute a state $\ket*{\alpha}_A$ to particle $q_A$, and a state $\ket*{\beta}_B$ to particle $q_B$, %
...
is the probability that $X$ occurs, $P(Y)$ is the probability that $Y$ occurs, and \DIFdelbegin \DIFdel{$P(X\,\&\,Y)$ }\DIFdelend \DIFaddbegin \DIFadd{$P(X, Y)$ }\DIFaddend is the probability that both $X$ and $Y$ both occur. We define the \textbf{conditional probability}\index{conditional probability} $P(X|Y)$ %
...
\end{equation}
From (\ref{indep}), it is easy to see that when $X$ and $Y$ are independent \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{with $P(Y)\neq 0$, then }\DIFaddend $P(X|Y)=P(X)$. We also say that two events $X$ and $Y$ are \textbf{conditionally independent}\index{conditional independence} given some third event $Z$ if and only if 
...
Now if $q_A$ can be described by the $\ket*{\alpha}_A$-state and $q_B$ can be described by the $\ket*{\beta}_B$-state, the Born Rule implies that the conditional probability \DIFaddbegin \DIFadd{(conditioned on $q_A$ being in the $\ket*{\alpha}_A$-state and $q_B$ being in the $\ket*{\beta}_B$-state) }\DIFaddend that Alice would measure her particle to be in the state $\ket*{\alpha'}_A$ is not going to depend on $\ket*{\beta}_B$,  and so  \DIFdelbegin \DIFdel{$P(\alpha'|\alpha\,\&\,\beta)=P(\alpha'|\alpha)$.Likewise, $P(\beta'|\alpha\,\&\,\beta)=P(\beta'|\beta)$}\DIFdelend \DIFaddbegin \DIFadd{$P(\alpha'|\alpha, \beta)=P(\alpha'|\alpha)$.}\footnote{ \DIFadd{$P(\alpha'|\alpha, \beta)$ denotes the conditional probability that $q_A$ will be measured to be in the $\ket*{\alpha'}_A$-state given that $q_A$ is in the $\ket*{\alpha}_A$-state and $q_B$ is in the $\ket*{\beta}_B$-state.}} \DIFadd{Likewise, $P(\beta'|\alpha, \beta)=P(\beta'|\beta)$}\DIFaddend . 
Therefore, if Alice and Bob's measurement outcomes are conditionally independent \DIFaddbegin \DIFadd{given the prior states of their particles}\DIFaddend , then according to (\ref{indepcond}) and the Born Rule, we would obtain the conditional probability
\begin{equation}\label{indepprob}
    P_{AB}(\alpha'\DIFdelbegin \DIFdel{\,\&\,}\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend \beta'|\alpha\DIFdelbegin \DIFdel{\,\&\,}\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend \beta)=|\ip{\alpha'}{\alpha}_A|^2\times|\ip{\beta'}{\beta}_B|^2=|\ip{\alpha'}{\alpha}_A\ip{\beta'}{\beta}_B|^2
\DIFdelbegin \DIFdel{.
}\DIFdelend \end{equation}
\DIFaddbegin \DIFadd{where $P_{AB}(\alpha', \beta'|\alpha, \beta)$ is the conditional probability Alice will measure $q_A$ to be in the $\ket*{\alpha'}_A$-state and Bob will measure $q_B$ to be in the $\ket*{\beta'}_B$-state given that $q_A$ is initially in the $\ket*{\alpha}_A$-state and $q_B$ is initially in the state $\ket*{\beta}$. }\DIFaddend %
...
\DIFadd{}\end{equation*}\DIFaddend 
\DIFaddbegin \DIFadd{It therefore follows that Alice }\DIFaddend and \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{Bob will measure their particles to have different spins with }\DIFaddend probability \DIFaddbegin \DIFadd{$1$, since for any proposition $q$ and its negation $\neg q$ (e.g. $q =$ }\DIFaddend Alice and Bob \DIFdelbegin \DIFdel{will }\DIFdelend measure their particles to have \DIFaddbegin \DIFadd{the same spin, and $\neg q = $ Alice and Bob measure their particles to have }\DIFaddend different spins\DIFdelbegin \DIFdel{will }\DIFdelend \DIFaddbegin \DIFadd{), the sum of the probabilities of $q$ and of $\neg q$ conditioned on any other proposition $\Psi$ that has non-zero probability (e.g. the particles $q_A$ and $q_B$ are in the $\ket*{\Psi_{\text{Bell}}}$ state) must }\DIFaddend be \DIFdelbegin \begin{displaymath}
\begin{split}
    \DIFdel{P_{AB}(\uvbp{a}\,\&\,\uvbm{a}|\Psi_{\text{Bell}})}&\DIFdel{+P_{AB}(\uvbm{a}\,\&\,\uvbp{a}|\Psi_{\text{Bell}})= }\\
    \DIFdel{=}&\DIFdel{\frac{1}{2}\big| \ip*{\uvbp{a}}{\uvbp{a}}_A\ip*{\uvbm{a}}{\uvbm{a}}_B-\ip*{\uvbp{a}}{\uvbm{a}}_A\ip*{\uvbm{a}}{\uvbp{a}}_B\big|^2}\\
    &\DIFdel{+\frac{1}{2}\big|\ip*{\uvbm{a}}{\uvbp{a}}_A\ip*{\uvbp{a}}{\uvbm{a}}_B-\ip*{\uvbm{a}}{\uvbm{a}}_A\ip*{\uvbp{a}}{\uvbp{a}}_B\big|^2=1.
}\end{split}
\DIFdel{}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{equal to $1$, i.e. we must have $P(q|\Psi)+P(\neg q|\Psi)=1.$}\DIFaddend } 
...
\end{align*}}Therefore, if Alice had chosen to measure her particle along the $\uvb{b}$-axis rather than the $\uvb{a}$-axis, she would still obtain equal probabilities for finding her particle to be in either the state $\ket*{\uvbp{b}}_A$ or $\ket*{\uvbm{b}}_A$, and the same equal probabilities for Bob's measurement outcomes hold as well. 
...
\nomenclature{$\sum_{i=1}^8 N_i$}{Shorthand for $N_1+N_2+N_3+N_4+N_5+N_6+N_7+N_8$, \nomrefpage}%
      ...
      $N_7$ & \DIFdelbeginFL \DIFdelFL{$\ket*{\uvbm{a},\uvbm{b},\uvbp{c}}_A$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\ket*{\uvbm{a};\uvbm{b};\uvbp{c}}_A$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$\ket*{\uvbp{a},\uvbp{b},\uvbm{c} }_B$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\ket*{\uvbp{a};\uvbp{b};\uvbm{c} }_B$}\DIFaddendFL \\ 

      $N_8$ & \DIFdelbeginFL \DIFdelFL{$\ket*{\uvbm{a},\uvbm{b},\uvbm{c}}_A$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\ket*{\uvbm{a};\uvbm{b};\uvbm{c}}_A$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$\ket*{\uvbp{a},\uvbp{b},\uvbp{c}}_B$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\ket*{\uvbp{a};\uvbp{b};\uvbp{c}}_B$ }\DIFaddendFL \\ 
      \hline
      \end{tabular}
      \label{hiddentable}
      \end{table}
\DIFdelbegin \DIFdel{As in section \ref{eprsec}, we define $P_{AB}(\uvbp{a}\,\&\,\uvbp{b})$   }\DIFdelend \DIFaddbegin \DIFadd{We define $P_{AB}(\uvbp{a},\uvbp{b})$   }\DIFaddend %
...
\DIFdelend \DIFaddbegin \nomenclature{$P_{AB}(\uvbp{a},\uvbp{b})$}{The probability particle $q_A$ is in the $\ket*{\uvbp{a}}$-state and particle $q_B$ is in the $\ket*{\uvbp{b}}$-state in the hidden variables interpretation, \nomrefpage}\DIFaddend %
to be the probability that Alice measures particle $q_A$ to be at location $\uvbp{a}$ on her detection screen and Bob measures particle $q_B$ to be at location $\uvbp{b}$ on his detection screen \DIFaddbegin \DIFadd{where this probability is conditioned only on the assumption that the two particles are prepared in such a way as to guarantee Alice and Bob obtain opposite spin measurement outcomes if they perform their measurements along the same axis}\DIFaddend . We similarly define the probabilities for all other combinations of detection locations. It is relatively easy to calculate all these probabilities in terms of the values $N_i$ from table \ref{hiddentable},\footnote{e.g.  \DIFdelbegin \DIFdel{$P_{AB}(\uvbp{a}\,\&\,\uvbp{b})=\frac{N_3+N_4}{N},\, P_{AB}(\uvbp{a}\,\&\,\uvbp{c})=\frac{N_2+N_4}{N},\,P_{AB}(\uvbp{c}\,\&\,\uvbp{b})=\frac{N_3+N_7}{N}$}\DIFdelend \DIFaddbegin \DIFadd{$P_{AB}(\uvbp{a},\uvbp{b})=\frac{N_3+N_4}{N},\, P_{AB}(\uvbp{a},\uvbp{c})=\frac{N_2+N_4}{N},\,P_{AB}(\uvbp{c},\uvbp{b})=\frac{N_3+N_7}{N}$}\DIFaddend .  } or alternatively by simply measuring the frequency of these different outcomes for where Alice and Bob detect their particles. Note that the values of $N_i$ will be unknown, but on the assumption that there is a fact of the matter of which states in table \ref{hiddentable} obtain, and on the assumption that the states to which the $N_i$ correspond exhaust all the possible states for Alice's and Bob's particles, we can show that\footnote{This inequality follows since 
...
\DIFdelend \DIFaddbegin $$\DIFadd{P_{AB}(\uvbp{a},\uvbp{b})=\frac{N_3+N_4}{N}\leq \frac{N_2+N_4+N_3+N_7}{N}=P_{AB}(\uvbp{a},\uvbp{c})+P_{AB}(\uvbp{c},\uvbp{b}).}$$\DIFaddend }
       \begin{equation}\label{bellinequality}
      P_{AB}(\uvbp{a}\DIFdelbegin \DIFdel{\,\&\,}\DIFdelend \DIFaddbegin \DIFadd{,}\DIFaddend \uvbp{b})\leq P_{AB}(\uvbp{a}\DIFdelbegin \DIFdel{\,\&\,}\DIFdelend \DIFaddbegin \DIFadd{,}\DIFaddend \uvbp{c})+P_{AB}(\uvbp{c}\DIFdelbegin \DIFdel{\,\&\,}\DIFdelend \DIFaddbegin \DIFadd{,}\DIFaddend \uvbp{b}).
      \end{equation}
...
\DIFdelbegin %DIFDELCMD < \nomenclature{$P_{\lambda,x,y}(X \,\&\, Y)$}{The probability  Alice obtains outcome $X$ and Bob obtains outcome $Y$ give the complete state $\lambda$ and Alice's measurement choice $x$ and Bob's measurement choice $y$, \nomrefpage}%%%
...
\begin{align}
P_{A, \lambda,x,y}(X)&=P_{\lambda,x,y}(X\DIFdelbegin \DIFdel{\,\&\, }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend 1)+P_{\lambda,x,y}(X\DIFdelbegin \DIFdel{\,\&\, }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend {-1}),\label{PIone}\\
P_{B, \lambda,x,y}(Y)&=P_{\lambda,x,y}(1\DIFdelbegin \DIFdel{\,\&\, }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend Y)+P_{\lambda,x,y}({-1}\DIFdelbegin \DIFdel{\,\&\, }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend Y),\label{PItwo}
\end{align}%
...
\DIFdelbegin \begin{displaymath}\DIFdel{P_{\lambda,\vb{\hat{a}},\vb{\hat{b}}}(\vb{\hat{a}+}\,\&\, \vb{\hat{b}+})+P_{\lambda,\vb{\hat{a}},\vb{\hat{b}}}(\vb{\hat{a}+}\,\&\,\vb{\hat{b}-})=\frac{1}{2}\abs*{\ip*{\vb{\hat{a}+}}{\vb{\hat{b}-}}_A}^2+\frac{1}{2}\abs*{\ip*{\vb{\hat{a}+}}{\vb{\hat{b}+}}_A}^2.}\end{displaymath}%DIFAUXCMD
...
Therefore, 
\DIFdelbegin \begin{displaymath}\DIFdel{P_{\lambda,\vb{\hat{a}},\vb{\hat{b}}}(\vb{\hat{a}+}\,\&\, \vb{\hat{b}+})+P_{\lambda,x,y}(\vb{\hat{a}+}\,\&\,\vb{\hat{b}-})=\frac{1}{2}.}\end{displaymath}%DIFAUXCMD
...
\DIFadd{Using a slightly more technical argument}\footnote{\DIFadd{The more technical argument is as follows: we work in one spatial dimension and suppose that Bob is travelling away from Alice with velocity $v$ on a trajectory $(t, vt)$  in Alice's spacetime coordinates. At time $t_A$, she sends a message to Bob which travels at a velocity $v_m>v$ so that her message can catch up with Bob. Her message to Bob therefore has a trajectory $(t_A+T,v_m T)$ where $T$ is the interval of time after she has sent her message. At the time $t_{AB}$ in Alice's time coordinates when her message reaches Bob, her coordinates for Bob and her coordinates for her message location must coincide. Therefore, 
}$$\DIFadd{(t_{AB}, v t_{AB})=(t_A +T_{AB}, v_m T_{AB}),}$$
\DIFadd{where $T_{AB}$ is the time it takes for her message to reach Bob as measured by Alice. Solving for $t_{AB}$, we therefore find that Alice's clock will show the time }$$\DIFadd{t_{AB}=\frac{t_A}{1-v/v_m}}$$ \DIFadd{at the moment Bob receives her message. But since Alice determines Bob's clock to be running more slowly than her own clock by a factor of $\gamma$, it follows that Bob's clock will show the time  
}$$\DIFadd{t_B=\frac{t_A}{\gamma(1-v/v_m)}}$$ \DIFadd{the moment he receives Alice's message. 
}


\DIFadd{Now suppose Bob immediately replies to Alice's message with his message propagating back to Alice with velocity $-v_m$. In Bob's coordinates, Alice will have a trajectory $(t,-vt)$, and Bob's message to her will have a trajectory $(t_B+T, -v_m T)$. Since these two trajectories must coincide at the moment Bob's reply reaches Alice, it follows that Bob's clock will show a time $t_{BA}$ which satisfies the equation
}$$\DIFadd{(t_{BA}, -v t_{BA})=(t_B+T_{BA}, -v_m T_{BA})}$$
\DIFadd{where $T_{BA}$ is the time it takes for Bob's message to Alice as measured by Bob. Solving for $t_{BA}$, we therefore find that Bob's clock will show the time
}$$\DIFadd{t_{BA}=\frac{t_B}{1-v/v_m}}$$
\DIFadd{when Alice receives Bob's reply. But since Bob determines Alice's clock to be running more slowly than his own clock by a factor of $\gamma$, it follows that Alice's clock will show the time  
}$$\DIFadd{t_{AA}=\frac{t_B}{\gamma(1-v/v_m)}=\frac{t_A}{\gamma^2(1-v/v_m)^2}}$$ \DIFadd{the moment she receives Bob's reply. Now an absurdity will only arise if $t_{AA}<t_A$ since then Alice would receive a reply from Bob before she had sent her message. Thus, an absurdity will arise if both $\gamma^2(1-v/v_m)^2>1$ and $v_m> v$, where as already mentioned, the second inequality must hold in order for Alice's message to reach Bob and vice versa. Now as we will see in footnote }\footreference{srdefivation}\DIFadd{, }$$\DIFadd{\gamma=\frac{1}{\sqrt{1-v^2/c^2}}.}$$ \DIFadd{A simple calculation therefore shows that the inequality $t_{AA}<t_A$ implies
}\begin{equation*}\DIFadd{\label{absurdinequality}
v>\frac{2v_m}{1+v_m^2/c^2}.
}\end{equation*}
\DIFadd{Combining this with the inequality $v<v_m$, we see that an absurdity will be possible if 
}$$ \DIFadd{v_m>\frac{2v_m}{1+v_m^2/c^2}}$$
\DIFadd{or equivalently, if $v_m>c$.}} 
\DIFadd{one can also see that an absurdity follows even when the signal transmission isn't almost instantaneous, just so long as the signal propagates faster than the speed of light. Special relativity therefore implies that it is impossible to send signals faster than the speed of light.
}

\DIFadd{Now a serious problem with PD is that it suggests the possibility of superluminal signalling. In particular, if Alice happened to know what $\lambda$ was for each run of the experiment, and if Bob made the same measurement, then because the distribution of Alice's outcomes will depend on Bob's choice of measurement, with enough runs of the experiment, Alice should be able to work out what measurement Bob is making. And this should be possible even if Alice and Bob are separated by many light years. So it seems that faster than light communication could be possible. The only thing preventing such communication would be Alice's lack of knowledge of $\lambda$. 
}

\DIFadd{One might still respond to this argument against PD by saying that in reality, Alice does not know anything about $\lambda$ and hence it won't be possible for Bob to send signals faster than the speed of light to Alice. However, it would nevertheless be very strange if the validity of special relativity hung on what human beings were capable of knowing rather than on the laws that actually governed physical reality itself. But even if it was metaphysically impossible for human beings to know $\lambda$, this would not allay the fears that adherents of special relativity would have against PD. For the real issue is not so much the possibility that Alice could translate a message that was transmitted from Bob at superluminal speed. Rather, the issue is that Bob could do something whose effect was propagated at superluminal speed. If PI fails, then superluminal propagation of effects will be possible.}\label{lambdaknowledge} \DIFadd{It's just that if Alice is to know the cause of this effect, she will need to know $\lambda$. But even if she doesn't know $\lambda$, it still seems as though some unknown effect has been transmitted to her superluminally, and with the superluminal propagation of an effect comes the possibility that an effect might occur before its cause. Such a possibility would be analogous to how Alice could receive a reply to a message from Bob before she had sent the original message to him.
%DIF >  flatex input end: [Chapter01/PDProblem.tex]
}

%DIF > \usepackage[inline]{showlabels}
\DIFaddend % flatex input: [Chapter01/PIPilot.tex]
...
One model which exhibits PD is the \textbf{\DIFdelbegin \DIFdel{pilot wave }\DIFdelend \DIFaddbegin \DIFadd{Bohmian }\DIFaddend interpretation}\DIFdelbegin %DIFDELCMD < \index{pilot wave interpretation} %%%
\DIFdelend \DIFaddbegin \index{Bohmian interpretation} \DIFaddend of standard quantum theory. In this interpretation, it is assumed that at any instant of time $t$, the particles $q_A$ and $q_B$ will have definite positions $\vb{x}_A$ and $\vb{x}_B$ %
\DIFdelbegin %DIFDELCMD < \nomenclature{$\vb{x}_A, \vb{x}_B$}{Exact locations of particles $q_A$ and $q_B$ in the pilot wave model, \nomrefpage}%%%
...
\DIFdelbegin %DIFDELCMD < \nomenclature{$\vb{p}_A, \vb{p}_B$}{Exact momenta of particles $q_A$ and $q_B$ in the pilot wave interpretation, \nomrefpage}%%%
...
\DIFdelbegin %DIFDELCMD < \nomenclature{$V$}{The region particles $q_A$ and $q_B$ are confined to in the pilot wave interpretation, \nomrefpage}%%%
...
\DIFdelbegin %DIFDELCMD < \nomenclature{$O(\vb{x}_A, \vb{x}_B)$}{A physical quantity dependent on the locations of the particles $q_A$ and $q_B$ in the pilot wave interpretation, \nomrefpage}%%%
...
that depends on the positions $\vb{x}_A$ and $\vb{x}_B$ of the two particles, then when the regions $V_i$ are  sufficiently small so that almost everywhere,\footnote{\DIFdelbegin \DIFdel{When $O(\vb{x}_A,\vb{x}_B)$ }\DIFdelend \DIFaddbegin \DIFadd{I am using the expression ``almost everywhere'' in a measure theoretic sense to mean that the set of all points where discontinuities occur has measure $0$. The motivation for this qualification }\DIFaddend is \DIFaddbegin \DIFadd{that whilst it might seem reasonable to suppose that there could be physical quantities that are }\DIFaddend discontinuous \DIFaddbegin \DIFadd{in some places}\DIFaddend , \DIFaddbegin \DIFadd{it doesn't seem reasonable to suppose that }\DIFaddend there \DIFdelbegin \DIFdel{may }\DIFdelend \DIFaddbegin \DIFadd{are physical quantities that could }\DIFaddend be \DIFaddbegin \DIFadd{pathologically discontinuous, e.g. we wouldn't expect there to be a physical quantity that took the value of $1$ unit when all the coordinates of a location were rational and a value of $2$ units when one of the coordinates was irrational. Given this qualification about $O(\vb{x}_A,\vb{x}_B)$ being continuous almost everywhere, then even though $O(\vb{x}_A,\vb{x}_B)$ could be discontinuous at }\DIFaddend some \DIFaddbegin \DIFadd{points so that there could be some  }\DIFaddend $V_i\times V_j$ cells in which $O(\vb{x}_A,\vb{x}_B)$ \DIFdelbegin \DIFdel{varies }\DIFdelend \DIFaddbegin \DIFadd{varied }\DIFaddend non-negligibly for $(\vb{x}_A,\vb{x}_B)\in V_i\times V_j$\DIFdelbegin \DIFdel{. But we assume }\DIFdelend \DIFaddbegin \DIFadd{, it would still be the case }\DIFaddend that the sum of the $p(V_i, V_j)\max_{(\vb{x}_i,\vb{x}_j)\in V_i\times V_j}|O(\vb{x}_i,\vb{x}_j)|$ terms for such cells \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{would be }\DIFaddend negligible. \DIFaddbegin \DIFadd{This follows from the Lebesgue-Vitali theorem that states that any bounded function on a compact interval is Riemann integrable if and only if it is continuous almost everywhere. See \mbox{%DIFAUXCMD
\cite{wiki:Riemann}}\hskip0pt%DIFAUXCMD
.}\DIFaddend } $O(\vb{x}_i, \vb{x}_j)$ varies negligibly for any $\vb{x}_i\in V_i$ and $\vb{x}_j\in V_j$,  the average value (also known as the \textbf{expectation value}\index{expectation value!pilot wave})
\begin{equation}\label{bohmconsistency}
\ev{O}=\sum_{i,j}p(V_i, V_j)O(\vb{x}_i, \vb{x}_j)
\end{equation}
calculated %
\DIFdelbegin %DIFDELCMD < \nomenclature{$\ev{O}$}{Expectation value of the physical quantity $O(\vb{x}_A, \vb{x}_B)$ in the pilot wave interpretation \nomrefpage}%%%
...
To see why PI fails to hold in the \DIFdelbegin \DIFdel{pilot wave }\DIFdelend \DIFaddbegin \DIFadd{Bohmian }\DIFaddend interpretation, we first note that since the \DIFdelbegin \DIFdel{pilot wave }\DIFdelend \DIFaddbegin \DIFadd{Bohmian }\DIFaddend interpretation makes the same predictions as standard quantum theory when averaged over all the hidden variables, the violation of Bell's inequality (\ref{bellinequality}) implies there must be some hidden variable $\lambda$ and choices of measurement directions $\bm{\hat{a}}$, $\bm{\hat{b}}$, and $\bm{\hat{c}}$ such that  %
...
P_{\lambda,\bm{\hat{a}},\bm{\hat{b}}}(\uvbp{a}\DIFdelbegin \DIFdel{\,\&\,}\DIFdelend \DIFaddbegin \DIFadd{,}\DIFaddend \uvbp{b})> P_{\lambda,\bm{\hat{a}},\bm{\hat{c}}}(\uvbp{a}\DIFdelbegin \DIFdel{\,\&\,}\DIFdelend \DIFaddbegin \DIFadd{,}\DIFaddend \uvbp{c})+P_{\lambda,\bm{\hat{c}},\bm{\hat{b}}}(\uvbp{c}\DIFdelbegin \DIFdel{\,\&\,}\DIFdelend \DIFaddbegin \DIFadd{,}\DIFaddend \uvbp{b}).\protect\footnotemark
...
\DIFdelbegin \begin{displaymath}\DIFdel{%DIFDELCMD < \label{bohmconsistency2}%%%
P_{AB}(\uvbp{a}\,\&\,\uvbp{b})=\sum_{\lambda}p_{\lambda}P_{\lambda,\bm{\hat{a}},\bm{\hat{b}}}(\uvbp{a}\,\&\,\uvbp{b})
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation}\DIFadd{\label{bohmconsistency2}
P_{AB}(\uvbp{a},\uvbp{b})=\sum_{\lambda}p_{\lambda}P_{\lambda,\bm{\hat{a}},\bm{\hat{b}}}(\uvbp{a},\uvbp{b})
}\end{equation}\DIFaddend  
where \DIFdelbegin \DIFdel{$P_{AB}(\uvbp{a}\,\&\,\uvbp{b})$ }\DIFdelend \DIFaddbegin \DIFadd{$P_{AB}(\uvbp{a},\uvbp{b})$ }\DIFaddend is the probability described in section \ref{hiddenbellsection}, $\lambda$ ranges over the pairs of indices $(i,j)$ corresponding to the cell $V_i\times V_j$, $p_{\lambda}=p(V_i,V_j)$ for $\lambda=(i,j)$, and we have used the fact that  \DIFdelbegin \DIFdel{$P_{\lambda,\bm{\hat{a}},\bm{\hat{b}}}(\uvbp{a}\,\&\,\uvbp{b})=O_{\uvbp{a},\uvbp{b}}(\vb{x}_i\,\&\,\vb{x}_j)$ }\DIFdelend \DIFaddbegin \DIFadd{$P_{\lambda,\bm{\hat{a}},\bm{\hat{b}}}(\uvbp{a},\uvbp{b})=O_{\uvbp{a},\uvbp{b}}(\vb{x}_i,\vb{x}_j)$ }\DIFaddend for almost all $(\vb{x}_i,\vb{x}_j)\in V_i\times V_j$. Now if (\ref{bellinequality2}) is false, then for all $\lambda$, $\bm{\hat{a}}$, and $\bm{\hat{b}}$,  
\DIFdelbegin \begin{displaymath}\DIFdel{%DIFDELCMD < \label{falsebellinequality2}%%%
...
\DIFdelbegin \begin{displaymath}\DIFdel{%DIFDELCMD < \label{falsebellinequality3}%%%
...
P_{\lambda,\bm{\hat{c}},\bm{\hat{b}}}(\uvbp{c}\DIFdelbegin \DIFdel{\,\&\,}\DIFdelend \DIFaddbegin \DIFadd{,}\DIFaddend \uvbp{b})&=0. \label{PDproof3}
 \end{align}
 We suppose that PI holds, and we will try to arrive at a contradiction.\DIFaddbegin \footnote{\DIFadd{For another proof of this result, see \mbox{%DIFAUXCMD
\cite{BellJ.S.1964OtEP}}\hskip0pt%DIFAUXCMD
.}} \DIFaddend If both Alice and Bob make their measurement in the $\bm{\hat{c}}$-direction, there are two possibilities: either Alice measures $q_A$ to be in the state $\uvbm{c}$ and Bob measures $q_B$ to be in the state $\uvbp{c}$, or Alice measures $q_A$ to be in the state $\uvbp{c}$ and Bob measures $q_B$ to be in the state $\uvbm{c}.$ So expressed in terms of probabilities, these two possibilities are equivalent to either 
 \begin{equation}\label{PDproofcase1}
 P_{\lambda,\bm{\hat{c}},\bm{\hat{c}}}(\uvbm{c}\DIFdelbegin \DIFdel{\,\&\,}\DIFdelend \DIFaddbegin \DIFadd{,}\DIFaddend \uvbp{c})=1 \qquad\text{and}\qquad  P\DIFdelbegin \DIFdel{_{\lambda,\bm{\hat{c}}\,\&\,\bm{\hat{c}}}}\DIFdelend \DIFaddbegin \DIFadd{_{\lambda,\bm{\hat{c}},\bm{\hat{c}}}}\DIFaddend (\uvbp{c},\uvbm{c})=\DIFdelbegin \DIFdel{0.
 }\DIFdelend \DIFaddbegin \DIFadd{0,
 }\DIFaddend \end{equation}
 or 
  \begin{equation}\label{PDproofcase2}
 P_{\lambda,\bm{\hat{c}},\bm{\hat{c}}}(\uvbp{c}\DIFdelbegin \DIFdel{\,\&\,}\DIFdelend \DIFaddbegin \DIFadd{,}\DIFaddend \uvbm{c})=1 \qquad\text{and}\qquad P\DIFdelbegin \DIFdel{_{\lambda,\bm{\hat{c}}\,\&\,\bm{\hat{c}}}}\DIFdelend \DIFaddbegin \DIFadd{_{\lambda,\bm{\hat{c}},\bm{\hat{c}}}}\DIFaddend (\uvbm{c},\uvbp{c})=\DIFdelbegin \DIFdel{0
 }\DIFdelend \DIFaddbegin \DIFadd{0.
 }\DIFaddend \end{equation}
 \DIFdelbegin \DIFdel{Let's first consider case }\DIFdelend \DIFaddbegin \DIFadd{It will be sufficient to show that }\DIFaddend (\ref{PDproofcase1}) \DIFdelbegin \DIFdel{. Note that
}\DIFdelend \DIFaddbegin \DIFadd{leads to a contradiction, since then we can just swap Alice and Bob to show that (\ref{PDproofcase2}) also leads to a contradiction. So note that
}\DIFaddend \begin{equation}
 P_{\lambda,\bm{\hat{c}},\bm{\hat{c}}}(\uvbp{c}\DIFdelbegin \DIFdel{\,\&\,}\DIFdelend \DIFaddbegin \DIFadd{,}\DIFaddend \uvbm{c})+P_{\lambda,\bm{\hat{c}},\bm{\hat{c}}}(\uvbm{c}\DIFdelbegin \DIFdel{\,\&\,}\DIFdelend \DIFaddbegin \DIFadd{,}\DIFaddend \uvbm{c})=0.
\end{equation}
 Therefore, since we are assuming PI, 
 \begin{equation}
 P_{\lambda,\bm{\hat{a}},\bm{\hat{c}}}(\uvbp{a}\DIFdelbegin \DIFdel{\,\&\,}\DIFdelend \DIFaddbegin \DIFadd{,}\DIFaddend \uvbm{c})+P_{\lambda,\bm{\hat{a}},\bm{\hat{c}}}(\uvbm{a}\DIFdelbegin \DIFdel{\,\&\,}\DIFdelend \DIFaddbegin \DIFadd{,}\DIFaddend \uvbm{c})=0.
\end{equation}
 In particular, 
...
\end{equation}
Since (\ref{PDproof5}) contradicts (\ref{PDproof6}), the assumption (\ref{PDproofcase1}) must be false if PI is to hold. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{So we now consider the alternative case when (\ref{PDproofcase2}) holds. We will again see that this assumption leads to a contradiction. First note that
}\begin{displaymath}
 \DIFdel{P_{\lambda,\bm{\hat{c}},\bm{\hat{c}}}(\uvbm{c}\,\&\,\uvbp{c})+P_{\lambda,\bm{\hat{c}},\bm{\hat{c}}}(\uvbm{c}\,\&\,\uvbm{c})=0.
}\end{displaymath}%DIFAUXCMD
%DIFDELCMD <  %%%
\DIFdel{By PI
}\begin{displaymath}
\DIFdel{P_{\lambda,\bm{\hat{c}},\bm{\hat{b}}}(\uvbm{c}\,\&\,\uvbp{b})+P_{\lambda,\bm{\hat{c}},\bm{\hat{b}}}(\uvbm{c}\,\&\,\uvbm{b})=0.
}\end{displaymath}%DIFAUXCMD
\DIFdel{In particular, 
}\begin{displaymath}\DIFdel{%DIFDELCMD < \label{PDproof8}%%%
P_{\lambda,\bm{\hat{c}},\bm{\hat{b}}}(\uvbm{c}\,\&\,\uvbp{b})=0.
}\end{displaymath}%DIFAUXCMD
%DIFDELCMD <    %%%
\DIFdel{But by (\ref{PDproof1}), we know that 
  }\begin{displaymath}
  \DIFdel{P_{\lambda,\bm{\hat{a}},\bm{\hat{b}}}(\uvbp{a}\,\&\,\uvbp{b})+P_{\lambda,\bm{\hat{a}},\bm{\hat{b}}}(\uvbm{a}\,\&\,\uvbp{b})=1,
  }\end{displaymath}%DIFAUXCMD
%DIFDELCMD <    
%DIFDELCMD <  %%%
\DIFdel{so using this together with PI, we must have
    }\begin{displaymath}\DIFdel{%DIFDELCMD < \label{PDproof7}%%%
  P_{\lambda,\bm{\hat{c}},\bm{\hat{b}}}(\uvbp{c}\,\&\,\uvbp{b})+P_{\lambda,\bm{\hat{c}},\bm{\hat{b}}}(\uvbm{c}\,\&\,\uvbp{b})=1.
  }\end{displaymath}%DIFAUXCMD
%DIFDELCMD <  
%DIFDELCMD <    %%%
\DIFdel{But by (\ref{PDproof3}) and (\ref{PDproof8})
}\begin{displaymath}\DIFdel{%DIFDELCMD < \label{PDproof9}%%%
 P_{\lambda,\bm{\hat{c}},\bm{\hat{b}}}(\uvbp{c}\,\&\,\uvbp{b})+P_{\lambda,\bm{\hat{c}},\bm{\hat{b}}}(\uvbm{c}\,\&\,\uvbp{b})=0.
}\end{displaymath}%DIFAUXCMD
\DIFdel{Since (\ref{PDproof7}) contradicts (\ref{PDproof9}), the assumption (}\DIFdelend \DIFaddbegin \DIFadd{Similarly, we will find that (}\DIFaddend \ref{PDproofcase2}) must \DIFdelbegin \DIFdel{also }\DIFdelend be false if PI is to hold. So we can only conclude that PI fails to hold in the \DIFdelbegin \DIFdel{pilot wave }\DIFdelend \DIFaddbegin \DIFadd{Bohmian }\DIFaddend interpretation.   But we can conclude even more than that: any deterministic hidden variable model that gives the same predictions as standard quantum theory when averaged over the hidden variables must violate PI.\label{PIdeterminism} 
...
%DIFDELCMD < %%%
\DIFdel{One might still respond to this argument against PD by saying that in reality, Alice does not know anything about $\lambda$ and hence it won't be possible for Bob to send signals faster than the speed of light to Alice. However, it would nevertheless be very strange if the validity of special relativity hung on what human beings were capable of knowing rather than on the laws that actually governed physical reality itself. But even if it was metaphysically impossible for human beings to know $\lambda$, this would not allay the fears that adherents of special relativity would have against PD. For the real issue is not so much the possibility that Alice could translate a message that was transmitted from Bob at superluminal speed. Rather, the issue is that Bob could do something whose effect was propagated at superluminal speed. If PI fails, then superluminal propagation of effects will be possible.}%DIFDELCMD < \label{lambdaknowledge} %%%
...
%DIFDELCMD < %%%
\DIFdelend % flatex input end: [Chapter01/PIPilot.tex]
...
\label{OIdet}In the case of deterministic models in which there are outcomes,\footnote{For deterministic models in which there are outcomes, the probability of a particular outcome given a complete description of a system will be either $0$ or $1$. For models in which there are no outcomes, such as the many-worlds interpretation which will be discussed in the next chapter, we can't speak of outcome independence.} OI necessarily holds. To see why, we \DIFdelbegin \DIFdel{first note that for deterministicmodels, either $P_{\lambda,x,y}(X,Y)=1$ or $P_{\lambda,x,y}(X,Y)=0$. When $P_{\lambda,x,y}(X,Y)=1$, then by (\ref{PIone})  }\DIFdelend \DIFaddbegin \DIFadd{note that since the model is deterministic}\DIFaddend , \DIFdelbegin \DIFdel{$P_{A, \lambda,x,y}(X)=1$, and by (\ref{PItwo}), }\DIFdelend \DIFaddbegin \DIFadd{either (1)  $P_{A, \lambda,x,y}(X)=$ }\DIFaddend $P_{B, \lambda,x,y}(Y)=1$, \DIFdelbegin \DIFdel{so (\ref{OI}) is seen to hold in this case. On the other hand, when $P_{\lambda,x,y}(X,Y)=0$,  if $P_{A, \lambda,x,y}(X)=1$, then by (\ref{PIone}), $P_{\lambda,x,y} (X,-Y)=1$ so that by (\ref{PItwo}), $P_{B, \lambda,x,y}(-Y)=1$, and hence $P_{B, \lambda,x,y}(Y)=0$ in which case (\ref{OI})holds. And similarly, if $P_{B, \lambda,x,y}(Y)=1$, by (\ref{PItwo}), $P_{\lambda,x,y} (-X,Y)=1$ so that by (\ref{PIone}), $P_{A, \lambda,x,y}(-X)=1$, and hence }\DIFdelend \DIFaddbegin \DIFadd{or (2) }\DIFaddend $P_{A, \lambda,x,y}(X)=0$ \DIFdelbegin \DIFdel{, so
again }\DIFdelend \DIFaddbegin \DIFadd{or $P_{B, \lambda,x,y}(Y)=0$. In case (1), then by definition of the probability $P_{\lambda,x,y}(X , Y)$, we clearly have $P_{\lambda,x,y} (X,Y)=1$, and so
}$$\DIFadd{P_{\lambda,x,y} (X,Y)=1=P_{A, \lambda,x,y}(X)P_{B, \lambda,x,y}(Y).}$$
\DIFadd{As for case (2), it clearly follows from the definition of the probability $P_{\lambda,x,y}(X , Y)$ that $P_{\lambda,x,y}(X , Y) = 0$, and so
}$$\DIFadd{P_{\lambda,x,y} (X,Y)=0=P_{A, \lambda,x,y}(X)P_{B, \lambda,x,y}(Y).}$$
\DIFadd{Therefore, }\DIFaddend (\ref{OI}) holds \DIFdelbegin \DIFdel{. And (\ref{OI}) obviously holds when $P_{A, \lambda,x,y}(X)=P_{B, \lambda,x,y}(Y)=0$. }\DIFdelend \DIFaddbegin \DIFadd{in either case. }\DIFaddend It therefore follows that OI holds in any deterministic model. In particular, OI holds under the \DIFdelbegin \DIFdel{pilot wave }\DIFdelend \DIFaddbegin \DIFadd{Bohmian }\DIFaddend interpretation. 

When it comes to the Copenhagen interpretation of quantum physics, however, OI fails to hold. For instance, if $x=y=\vb{\hat{a}}$, then $P_{\lambda,\vb{\hat{a}},\vb{\hat{a}}}(\vb{\hat{a}+},\vb{\hat{a}+})=0,$ but \DIFdelbegin \DIFdel{$P_{A, \lambda,\vb{\hat{a}},\vb{\hat{a}}}(\vb{\hat{a}+})=P_{B, \lambda,\vb{\hat{a}},\vb{\hat{a}}}(\vb{\hat{a}+})=1/2.$}\DIFdelend \DIFaddbegin \DIFadd{$P_{A, \lambda,\vb{\hat{a}},\vb{\hat{a}}}(\vb{\hat{a}+})=P_{B, \lambda,\vb{\hat{a}},\vb{\hat{a}}}(\vb{\hat{a}+})=1/2,$}\DIFaddend \footnote{See footnote \footreference{onehalf}. } \DIFdelbegin \DIFdel{Hence, OI fails}\DIFdelend \DIFaddbegin \DIFadd{and so in this case, (\ref{OI}) is false}\DIFaddend . 
...
So far we've seen that PI holds in the Copenhagen interpretation (section \ref{PISec}), that OI holds in the \DIFdelbegin \DIFdel{pilot wave }\DIFdelend \DIFaddbegin \DIFadd{Bohmian }\DIFaddend interpretation (section \ref{OISec}), and that due to the violation of Bell's inequality, it is not possible for both OI and PI to hold in a model that is consistent with physical reality (section \ref{OISec}). Nevertheless, as long as PI holds, the failure of OI does not enable Bob to send messages to Alice faster than light because Bob only has control over the measurement choice he makes rather than the outcome he observes. Assuming Bob's mental states have no effect on the measurement outcome, there is nothing he can do to influence his outcome, so although Alice will be able to work out \DIFaddbegin \DIFadd{or at least get some information about }\DIFaddend Bob's measurement outcome if she already happens to know which choice of measurement he has made, she will not be able to work out which measurement Bob makes (or even whether Bob has made a measurement at all) by measuring the outcome of her particle. For Shimony,\footnote{See \cite[146-147]{Shimony86}.} this inability to send \DIFdelbegin \DIFdel{super-luminal }\DIFdelend \DIFaddbegin \DIFadd{superluminal }\DIFaddend messages between Alice and Bob when PI holds and OI is violated was deemed sufficient for  standard quantum theory and special relativity to peacefully coexist.  

However, Butterfield is not satisfied with Shimony's solution to peaceful coexistence.\footnote{See \cite[p. 12]{Butterfield}.} Firstly, he notes that proofs of \DIFdelbegin \DIFdel{non-super-luminal }\DIFdelend \DIFaddbegin \DIFadd{non-superluminal }\DIFaddend signaling\footnote{e.g. see \cite[p. 113--116]{Redhead}; \cite[p. 139--140]{Hiley}} make no assumptions about spacetime locations.\footnote{For a definition of a spacetime location, see page \pageref{spacetimedef}.} One would have thought that any proof that \DIFdelbegin \DIFdel{super-luminal }\DIFdelend \DIFaddbegin \DIFadd{superluminal }\DIFaddend signalling between two points is impossible would have to show that a signal cannot be transmitted from one point to the other in less time than the time it takes light to travel between the two points. But if nothing is said about the location of these two points or what is so special about the speed of light compared to the speed of any other particle, then there does not seem to be enough information in the premises to draw the desired conclusion that \DIFdelbegin \DIFdel{super-luminal }\DIFdelend \DIFaddbegin \DIFadd{superluminal }\DIFaddend signaling is impossible in quantum physics.
...
\DIFaddbegin \DIFadd{These three parts of the measurement problem will be discussed in some detail later on in this chapter. }\DIFaddend Decoherence theory is able to resolve parts (1) and (2) of the measurement problem, and following Schlosshauer, we will explain how this is done, but as we will see (also following Schlosshauer), decoherence theory is not able to resolve part (3) of the measurement problem, the problem of outcomes.
...
\section{A Preliminary Consideration of the Many-Worlds Interpretation}
     In the previous chapter, we \DIFaddbegin \DIFadd{considered experiments for which there are only two possible measurement outcomes (given the choice of measurement made), and we }\DIFaddend saw that in order to calculate the probabilities of \DIFdelbegin \DIFdel{the various measurement outcomesof an experiment}\DIFdelend \DIFaddbegin \DIFadd{these two outcomes}\DIFaddend , we can do this by positing that the state describing the item being measured is in a superposition of two states each of which corresponds to a definite measurement outcome. For example, in the case of a spin singlet, the quantum state of the two particles can be expressed as the Bell state
     \begin{equation}\tag{\ref{bell} revisited}          
          \ket*{\Psi_{\text{Bell}}}=\frac{1}{\sqrt{2}}(\ket*{\uvbp{a}}_A\ket*{\uvbm{a}}_B-\ket*{\uvbm{a}}_A\ket*{\uvbp{a}}_B) 
      \DIFdelbegin \DIFdel{. 
      }\DIFdelend \end{equation}
...
\item $\forall \lambda,\mu\in\mathbb{C}$ (i.e. in the set of complex numbers -- this is why it is called a \emph{complex} vector space), and $\psi\in V,\,\lambda(\mu\psi)=(\lambda\mu)\psi$\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{,
}\DIFaddend \item for the scalar $1\in\mathbb{C},\, 1\psi=\psi,\,\forall\psi\in V$\DIFaddbegin \DIFadd{,
}\DIFaddend \item $\lambda(\psi+\chi)=\lambda\psi+\lambda\chi,\forall\psi,\chi\in V$ and $\lambda\in\mathbb{C}$
...
\begin{itemize}[topsep=0pt]
\item $\ip{\psi}{\chi}\in\mathbb{C},\,\forall\psi,\chi\in H$\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{,
...
\DIFdelbegin \begin{align*}\DIFdel{\ev*{\hat{O}}{\psi}}&\DIFdel{= \ev*{\hat{O} I}{\psi}=\sum_{i=1}^N \mel{\psi}{\hat{O}}{\psi_i}\ip{\psi_i}{\psi}=\sum_{i=1}^No_i\ip{\psi}{\psi_i}\ip{\psi_i}{\psi}}\\&\DIFdel{=\sum_{i=1}^No_i\abs{\ip{\psi_i}{\psi}}^2=\sum_{i=1}^No_ip_i=\ev*{\hat{O}}_\psi.}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align*}\DIFadd{\ev*{\hat{O}}{\psi}}&\DIFadd{= \ev*{\hat{O} I}{\psi}=\sum_{i=1}^N \mel{\psi}{\hat{O}}{\psi_i}\ip{\psi_i}{\psi}=\sum_{i=1}^No_i\ip{\psi}{\psi_i}\ip{\psi_i}{\psi}=\sum_{i=1}^No_i\abs{\ip{\psi_i}{\psi}}^2}\\&\DIFadd{=\sum_{i=1}^No_ip_i=\ev*{\hat{O}}_\psi.}\end{align*}\DIFaddend }We can see that this formula is correct in the simple example when the system is in the $\ket*{\psi_i}$-state, for in this case, the expectation value should be $o_i$, and we clearly have $\ev*{\hat{O}}{\psi_i}=o_i$ since $\hat{O}\ket*{\psi_i}=o_i\ket*{\psi_i}$ and $\ip*{\psi_i}=1$. Hence, $\ev*{\hat{O}}_{\psi_i}=\ev*{\hat{O}}{\psi_i}$ as expected.
...
\DIFdelbegin %DIFDELCMD < \nomenclature{$\propto$}{Means proportional to, e.g. $\ket*{\psi_1}_\mathcal{S}\propto\ket*{\psi_2}_\mathcal{S}$ means there exists $\lambda$ such that  $\ket*{\psi_1}_\mathcal{S}=\alpha\ket*{\psi_2}_\mathcal{S}$,  \nomrefpage}%%%
...
\DIFdelbegin %DIFDELCMD < \nomenclature{$\hat{O}_\rho$}{The expectation value  for an observable $\hat{O}$ of a system given that the density matrix $\hat{\rho}$ describes all we know about the system, see equation (\ref{expdensity}), \nomrefpage}%%%
\DIFdelend \DIFaddbegin \nomenclature{$\ev*{\hat{O}}_\rho$}{The expectation value  for an observable $\hat{O}$ of a system given that the density matrix $\hat{\rho}$ describes all we know about the system, see equation (\ref{expdensity}), \nomrefpage}\DIFaddend % 
    ...
    $$\Tr_\mathcal{S}(\hat{A}_\mathcal{S})=\sum_i \ev*{\hat{A}_\mathcal{S}}{\psi_i},$$
     ...
     \DIFdelbegin \begin{displaymath}\DIFdel{\Tr_\mathcal{U}(\hat{A}_\mathcal{U})=\sum_{ij} \ev*{\hat{A}_\mathcal{U}}{\xi_{ij}}.}\end{displaymath}%DIFAUXCMD
...
\nomenclature{$\Tr_\mathcal{E}(\hat{A}_\mathcal{U})$}{The partial trace of an operator $\hat{A}_\mathcal{U}$ acting on $H_\mathcal{U}$,  see equation (\ref{partialtrace}), \nomrefpage}% 
    ...
    \DIFdelbegin \begin{displaymath}\DIFdel{\hat{A}_\mathcal{U}=\sum_{ijkl}\mu_{ijkl}\dyad{\xi_{ij}}{\xi_{lk}},}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation}\DIFadd{\label{Au}
    ...
    we can use equation (\ref{partialtrace2}) to find that 
     \DIFdelbegin \begin{displaymath}\DIFdel{\Tr_\mathcal{E}(\hat{A}_\mathcal{U})=\sum_{ijl}\mu_{ijjl}\dyad{\psi_i}{\psi_l}.}\end{displaymath}%DIFAUXCMD
...
% flatex input: [Chapter02/Many-WorldsInterpretation.tex]
    ...
    Not everyone is convinced that the problem of outcomes is a genuine problem. In particular, \DIFdelbegin \DIFdel{people }\DIFdelend \DIFaddbegin \DIFadd{physicists such as Everett, De Witt, Deutsch, Zeh, and Zurek}\footnote{\DIFadd{For a short history of the development of the many-worlds interpretation, see \mbox{%DIFAUXCMD
\cite[336--337]{Schlosshauer}}\hskip0pt%DIFAUXCMD
. The central idea underlying the many-worlds was first described in \mbox{%DIFAUXCMD
\cite{EverettHugh1957RSFo}}\hskip0pt%DIFAUXCMD
. Everett's idea and the idea of branching worlds was popularized in \mbox{%DIFAUXCMD
\cite{DeWittBryceS.1970Qmar} }\hskip0pt%DIFAUXCMD
and \mbox{%DIFAUXCMD
\cite{DEUTSCHD1985Qtaa}}\hskip0pt%DIFAUXCMD
. Zeh and Zurek used Everett's ideas in their development of decoherence theory, e.g. see \mbox{%DIFAUXCMD
\cite{ZehH.D.1993Tanq} }\hskip0pt%DIFAUXCMD
and \mbox{%DIFAUXCMD
\cite{ZurekWojciechH.1998Deat}}\hskip0pt%DIFAUXCMD
.}} \DIFaddend who endorse the many-worlds interpretation of quantum physics effectively argue that there are no outcomes in the traditional sense. In this section, we give an account of the many-worlds interpretation of quantum physics and why physicists find it attractive. To this end, let us consider a physical universe $\mathcal{U}=\mathcal{S}+\mathcal{A}+\mathcal{P}_A+\mathcal{P}_B+\mathcal{E}$ consisting of subsystems $\mathcal{S}, \mathcal{A}, \mathcal{P}_A,\mathcal{P}_B$  %
...
\nomenclature{$\ket*{p_{i, A}(t)}, \ket*{p_{i, B}(t)}$}{The states of Alice and Bob after they have observed the apparatus to be in the state $\ket*{a_i(t)}$, \nomrefpage}%
    ...
    Now suppose the initial (normalized) state of $\mathcal{S}$ is $\ket*{\psi}=\sum_i c_i\ket*{s_i}$, so that the state for the composite system \DIFdelbegin \DIFdel{$\mathcal{U}$ is $\ket*{\Psi(t)}=\sum_i c_i \ket*{\xi_i(t)}\ket*{E_i(t)}$ where $\ket*{\xi_i(t)}=\ket*{s_i}\ket*{a_i(t)}\ket*{p_{i,A}(t)}\ket*{p_{i,B}(t)}$. }\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{U}=\mathcal{V}+\mathcal{E}$ is $\ket*{\Psi(t)}=\sum_i c_i \ket*{\xi_i(t)}\ket*{E_i(t)}\in H_{\mathcal{U}}$ where $\ket*{\xi_i(t)}=\ket*{s_i}\ket*{a_i(t)}\ket*{p_{i,A}(t)}\ket*{p_{i,B}(t)}\in H_{\mathcal{V}}$. %DIF > 
     ...
     \DIFaddend We also define the corresponding density matrix for the composite system $\hat{\rho}=\dyad{\Psi}$. If we were unable to make any observations on $\mathcal{E}$, then 
     the partial trace
...
\nomenclature{$W_i$}{A world described by the state $\ket*{\xi_i(t)}\ket*{E_i(t)}$,  \nomrefpage}%
    ...
    \begin{align*}\ket*{\text{Cat Alive}}= &\frac{1}{\sqrt{2}}\Big(\frac{1}{\sqrt{2}}(\ket*{\text{Cat Alive}}+\ket*{\text{Cat Dead}})\Big)\\&+\frac{1}{\sqrt{2}}\Big( \frac{1}{\sqrt{2}}(\ket*{\text{Cat Alive}}-\ket*{\text{Cat Dead}})\Big) \DIFdelbegin \DIFdel{. }\DIFdelend \end{align*} does not incline us to think of the state $\ket*{\text{Cat Alive}}$ as being composed of the mutually exclusive cat states  $\frac{1}{\sqrt{2}}(\ket*{\text{Cat Alive}}+\ket*{\text{Cat Dead}})$ and $\frac{1}{\sqrt{2}}(\ket*{\text{Cat Alive}}-\ket*{\text{Cat Dead}})$. 
    ...
    \begin{equation}\label{manyapprox}\ev*{\hat{O}_\mathcal{U}}_{\Psi(t)}\approx\sum_i |c_i|^2 \ev*{\hat{O}_\mathcal{V}}_{\xi_i(t)}\DIFdelbegin \DIFdel{.}\DIFdelend \end{equation}
    \DIFaddbegin \DIFadd{where $\hat{O}_\mathcal{U}$ is the natural extension of $\hat{O}_\mathcal{V}$ to $H_\mathcal{U}$ analogous to the extension given in equation (\ref{extension}) with $\mathcal{S}$ replaced by $\mathcal{V}$. 
    }\DIFaddend The fact that (\ref{manyapprox}) is only an approximation suggests that the time at which branching occurs is not well-defined. All that we can do is choose a  sufficiently large time interval $\delta$ such that for  $t>t_0+\delta$, the approximation (\ref{manyapprox}) meets our desired level of accuracy. 
...
.}} \DIFadd{And even if we can give some meaning to the states of the Hilbert space for the universe, there will never be the kinds of states for subsystems of the universe that we would expect there to be. For instance, there would never be states such as $\ket*{\text{living cat}}$ describing a physical system constituting a cat. Rather, there would only be states of the form  
      }\begin{equation}\DIFadd{\label{notacat}
      \alpha\ket*{\text{living cat}}\ket*{\chi}+\beta\ket*{\text{not a living cat}}\ket*{\chi'}
    }\end{equation} \DIFadd{for non-zero $\alpha$ and $\beta$, and (\ref{notacat}) is not the state of a cat. Thus, the many-worlds interpretation implies that 
    no cats exist.
}

      
      \DIFaddend Another reason for rejecting the many-worlds interpretation is that intuitively, it seems obvious that I can know I am alive without needing to know the state of the rest of the universe, but the many-worlds interpretation does not allow me to make this absolute matter of fact claim. 
...
%DIFDELCMD < %%%
\DIFdelend %DIF > \usepackage[inline]{showlabels}
\DIFaddbegin \newpage 
\DIFaddend % flatex input: [Chapter03/chapterIntro.tex]
...
\DIFaddend % flatex input end: [Chapter03/chapterIntro.tex]
...
We first consider the no-collapse feature of Kent's theory. This is a feature that belongs both to the many-worlds interpretation and to the \DIFdelbegin \DIFdel{pilot wave }\DIFdelend \DIFaddbegin \DIFadd{Bohmian }\DIFaddend interpretation. In all three interpretations, the quantum state deterministically evolves according to the Schr\"{o}dinger equation. The Schr\"{o}dinger equation itself describes how a quantum state evolves over time when there are no outside influences. The precise formula for the Schr\"{o}dinger equation need not concern us here, but all we need to know is that the Schr\"{o}dinger equation determines a so-called \textbf{unitary operator}\index{unitary operator} $U(t',t)$. %
...
Now a key insight of special relativity is that there is no such thing as absolute time. So for instance, two spacetime locations might seem to be simultaneous from one frame of reference, but another person travelling at a different velocity would judge with equal propriety the same two spacetime locations to be non-simultaneous. But it is not the case that for any two spacetime locations we can always find a frame of reference in which the two spacetime locations are simultaneous -- sometimes this is not possible. \DIFdelbegin \DIFdel{But we refer to }\DIFdelend \DIFaddbegin \DIFadd{We refer to distinct }\DIFaddend spacetime locations that could be simultaneous in some frame of references as being \textbf{spacelike-separated}\index{spacelike-separation}. For example, the two spacetime locations $o$ and \DIFdelbegin \DIFdel{$a           $  }\DIFdelend \DIFaddbegin \DIFadd{$a$  }\DIFaddend in figure \ref{cone} are spacelike-separated. 
...
\centering
 ...
 \end{tikzpicture}
 \DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Meaning of spacelike, timelike and lightlike-separation]{\DIFaddendFL The meaning of spacelike, timelike and lightlike-separation when there are two space dimensions and one time dimension.}
 \label{cone}
\end{figure}
There are also \DIFaddbegin \DIFadd{distinct }\DIFaddend spacetime locations in spacetime that could be connected by a beam of light such as the two spacetime locations $o$ and $b$ in figure \ref{cone}. Such spacetime locations are referred to as being \textbf{lightlike-separated}\index{lightlike-separation}. For any given spacetime location, the spacetime locations that are lightlike-separated from it form two cones\footnote{Strictly speaking, the set of spacetime locations that are lightlike-separated from a given spacetime location form the surface of a cone rather than a cone (which is a convex object). But among physicists, the terminology light cone has stuck.} called the future light cone and the past light cone as shown in figure \ref{cone}. Because light appears to travel at the same speed no matter what frame of reference one uses, the light cone of a spacetime location remains invariant when one changes from one reference frame to another. In other words, if another spacetime location lies on the light cone of a spacetime location in one frame of reference, then it must lie on the light cone of this spacetime location in every frame of reference. 
...
Now Kent supposes that a notional measurement is made over the whole of $S$ to determine a measurement outcome for $T_S(x)$ which we denote as $\tau_S(x)$ for every $x\in S$.  %DIF < 
 \DIFdelbegin %DIFDELCMD < \nomenclature{$\tau_S(x)$}{A real valued function which specifies the outcome of the notional measurement $T_S(x)$ for every $x\in S$, \nomrefpage}%%%
%DIF < 
  \DIFdelend It is these $\tau_S(x)$ that are the additional variables of his theory. \DIFaddbegin \DIFadd{How this determination of $T_S(x)$ comes about is up to one's philosophical preferences. For example, one could suppose that it was simply by divine fiat that this determination  of $T_S(x)$ comes about.}\footnote{\DIFadd{I will discuss my philosophical preference in the final chapter.}} 
 %DIF > 
 \nomenclature{$\tau_S(x)$}{A real valued function which specifies the outcome of the notional measurement $T_S(x)$ for every $x\in S$, \nomrefpage}%DIF > 
 \DIFaddend 


Clearly,  Kent is not assuming that such a measurement could be performed on $S$ in reality, hence his reason for referring to the measurement as fictitious/notional.\footnote{Kent uses the word fictitious when referring to these measurements, e.g. \cite[3]{Kent2014}. Butterfield refers to these measurements as notional e.g. \cite[17]{Butterfield}.} However, Kent does seem to be saying that there would be a fact of the matter about what value $T_S(x)$ would have approximately had at each $x\in S$ if it had been measured.\footnote{\label{asymptoticfootnote}Kent talks about taking the asymptotic limit as $S$ tends to the infinite future of $S_0$. In saying this, he supposes that the physics we are interested in happens between two hypersurfaces $S_0$ and $S_1$. Then given some measurement outcome $\tau_S$ on a hypersurface $S$ after $S_1$, for every hypersurface $S'$ after $S$, there is a range of possible measurements $\tau_{S'}$ on $S'$ that together occur with approximately the same probability as the measurement outcome of $\tau_S$ on $S$ and such that the beables between $S_0$ and $S_1$ determined by any one of the $\tau_S'$ outcomes on $S'$ in this range would be approximately the same as the beables determined by $\tau_S$ on $S$. See \cite[3]{Kent2014}.} \DIFdelbegin \DIFdel{So although }\DIFdelend \DIFaddbegin \DIFadd{Given that there is some fact of the matter about the value $\tau_S(x)$ that $T_S(x)$ takes for every $x\in S$ were $T_S(x)$ to be measured, Kent constructs a picture of physical reality that evolves in a way that is consistent with $T_S(x)$ being $\tau_S(x)$ for $x\in S$. 
}

\DIFadd{Kent's proposal might initially sound like he is suggesting that physical reality evolves unitarily from an initial state of the universe $\ket*{\Psi_0}$}\label{Psi0PsiSdef} \DIFadd{describing a hypersurface $S_0$ to a state $\ket*{\Psi_S}$ describing the hypersurface }\DIFaddend $S$\DIFdelbegin \DIFdel{is in the far distant future, one }\DIFdelend \DIFaddbegin \DIFadd{, and then a measurement of $T_S(x)$ is made yielding a value $\tau_S(x)$ for all $x\in S$ which in turn  changes the past by a process of backwards causality so that the history of the universe evolving from $S_0$ is now consistent with $T_S(x)$ having the determinate value $\tau_S(x)$. However, it is not necessary to think that Kent's interpretation requires backwards causality. Rather, we }\DIFaddend can think of the values $\tau_S(x)$ for $T_S(x)$ as being determined primordially \DIFdelbegin \DIFdel{, and as a consequence of this primordial determination, physical reality will evolve in a particular way }\DIFdelend \DIFaddbegin \DIFadd{before the evolution of physical reality begins. In other words, we first suppose that the universe is initially in a determinate state $\ket*{\Psi_0}$ that describes a hypersurface $S_0$. Given this state, there are many possibilities for the measurement outcome $T_S(x)$ on the distant future hypersurface $S$ that can be worked out from $\ket*{\Psi_S}$, but we don't consider $\ket*{\Psi_S}$ as describing the state of actuality of the hypersurface $S$ -- all $\ket*{\Psi_S}$ does is describe possibilities. One of the possible outcomes $\tau_S(x)$ is then selected,}\footnote{\DIFadd{Kent doesn't say anything about the selection of $\tau_S(x)$ beyond its selection being consistent with the Born Rule.}} \DIFadd{and then physical reality evolves in a way that is }\DIFaddend consistent with $T_S(x)$ being $\tau_S(x)$ if a measurement were to be made on $S$. \DIFaddbegin \DIFadd{One can therefore avoid positing the need for backwards causality by insisting that the states that unitarily evolve from $S_0$ don't describe the actual state of the universe. Rather, there is just one history,}\footnote{\DIFadd{The role that $\tau_S(x)$ plays in determining this one history will be described in sections \ref{OneWorldFeature} and  \ref{kentcalculation}.}} \DIFadd{and this history is consistent with $T_S(x)$ being $\tau_S(x)$ for $x\in S$.
}\DIFaddend 
...
\nomenclature{$\ket*{\Phi}$}{A state in the Heisenberg picture, \nomrefpage}%
 ...
 \nomenclature{$\hat{\bm{O}}(t)$}{A time dependent observable in the Heisenberg picture, \nomrefpage}%
  ...
  \nomenclature{$\hat{\bm{O}}(t, \bm{x})$}{An observable in the Heisenberg picture at a particular time $t$ and spatial location $\bm{x}$, \nomrefpage}%
   is an observable of some quantity at a particular time $t$ and spatial location $\bm{x}$.\footnote{For example, in quantum electrodynamics (which is one kind of quantum field theory), the observables will be expressible in terms of fields such as the \emph{four-vector potential}\index{four-vector potential}  \DIFdelbegin \DIFdel{potential }\DIFdelend $A^\mu(x)$  %
...
The commutativity of all the $\hat{T}_S(x)$ for $x\in S$ means that if \DIFdelbegin \DIFdel{$\ket*{\Psi}\in H_S$ }\DIFdelend \DIFaddbegin \DIFadd{$\ket*{\Gamma}\in H_S$ }\DIFaddend is an eigenstate of $\hat{T}_S(x)$, then for any $y\in S$, \DIFdelbegin \DIFdel{$\hat{T}_S(y)\ket*{\Psi}$ }\DIFdelend \DIFaddbegin \DIFadd{$\hat{T}_S(y)\ket*{\Gamma}$ }\DIFaddend is also an eigenstate of  $\hat{T}_S(x)$ with the same eigenvalue as \DIFdelbegin \DIFdel{$\ket*{\Psi}$.}\DIFdelend \DIFaddbegin \DIFadd{$\ket*{\Gamma}$.}\footnote{\DIFadd{To see this, suppose that $\tau$ is the eigenvalue of $\hat{T}_S(x)$ corresponding to the eigenstate $\ket*{\Gamma}$. Then by 
...
\DIFdelend \DIFaddbegin \nomenclature{$\{\ket*{\Gamma_{i}}:i\in\mathbb{N}\}$}{Orthonormal basis of $H_S$ consisting of simultaneous $\hat{T}_S$-eigenstates with corresponding simultaneous $\hat{T}_S$-eigenvalues $\tau_{S,i}(x)$, \nomrefpage}\DIFaddend %
 of $H_S$ \DIFdelbegin \DIFdel{such that
  $\hat{T}_S(x)\ket*{\Psi^{(i)}}=\tau^{(i)}_S(x)\ket*{\Psi^{(i)}}$ }\DIFdelend \DIFaddbegin \DIFadd{(where $\mathbb{N}$ is the set of integers greater than $0$) 
 }\nomenclature{$\mathbb{N}$}{ the set of positive integers greater than $0$, \nomrefpage}%DIF >  
 \DIFadd{such that
  $\hat{T}_S(x)\ket*{\Gamma_{i}}=\tau_{S,i}(x)\ket*{\Gamma_{i}}$ }\DIFaddend for all $x\in S$, where \DIFdelbegin \DIFdel{$\tau^{(i)}_S(x)\geq 0$ }\DIFdelend \DIFaddbegin \DIFadd{$\tau_{S,i}(x)\geq 0$ }\DIFaddend is  %
...
\DIFdelend \DIFaddbegin \nomenclature{$\tau_{S,i}(x)$}{Simultaneous $\hat{T}_S$-eigenvalue corresponding to simultaneous $\hat{T}_S$-eigenvalue $\ket*{\Gamma_{i}}$, \nomrefpage}\DIFaddend %
   a possible mass-energy density measurement defined for every $x$ in $S$. We will refer to a state \DIFdelbegin \DIFdel{$\ket*{\Psi}$ }\DIFdelend \DIFaddbegin \DIFadd{$\ket*{\Gamma}\in H_S$ }\DIFaddend as a \textbf{simultaneous $\hat{T}_S$-eigenstate}\index{simultaneous $\hat{T}_S$-eigenstate}, and a real valued function $\tau_S$ defined on the whole of $S$ as a  \textbf{simultaneous $\hat{T}_S$-eigenvalue}\index{simultaneous $\hat{T}_S$-eigenvalue} if and only if \DIFdelbegin \DIFdel{$\hat{T}_S(x)\ket*{\Psi}=\tau_S(x)\ket*{\Psi}$ }\DIFdelend \DIFaddbegin \DIFadd{$\hat{T}_S(x)\ket*{\Gamma}=\tau_S(x)\ket*{\Gamma}$ }\DIFaddend for all $x\in S$.\label{simultaneous}

 At this point, it is worth clarifying the different meanings of $T_S(x)$, $\hat{T}_S(x)$, and $\tau_S(x)$.\label{TSclarification} We use $T_S(x)$ to refer to the description of the physical quantity that is being observed. Thus, $T_S(x)$ is shorthand for the description ``the mass-energy density of the hypersurface $S$ observed at spacetime location $x$''. \DIFdelbegin \DIFdel{The function }\DIFdelend \DIFaddbegin \DIFadd{We use }\DIFaddend $\tau_S(x)$ \DIFdelbegin \DIFdel{stands for a particular range of values for each $x\in S$ of }\DIFdelend \DIFaddbegin \DIFadd{to stand for a real valued function on $S$ that could be a measurement outcome of }\DIFaddend the physical quantity described by $T_S(x)$ \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{for each $x\in S$. Thus, the equation $T_S=\tau_S$ is shorthand for the statement ``the mass-energy density of the hypersurface $S$ observed at spacetime location $x$ is $\tau_S(x)$ for every $x$ belonging to $S$.'' }\DIFaddend And for each $x\in S$, $\hat{T}_S(x)$ is the observable (i.e. the Hermitian operator) such that if an observer deems $S$ to be in an eigenstate $\ket*{\psi}$ of  $\hat{T}_S(x)$ with eigenvalue $\tau$ (a real number), then the observer would observe the physical quantity described by  $T_S(x)$ to have the value $\tau$. We will add a further clarification to this when we come to consider different observers in section \ref{LorentzInvariance}.
...
\hat{T}_S(c_x)\DIFdelbegin %DIFDELCMD < \ket*{\Psi^{(i)}}%%%
\DIFdelend \DIFaddbegin \ket*{\Gamma_{i}}\DIFaddend = \tau\DIFdelbegin \DIFdel{_S^{(i)}}\DIFdelend \DIFaddbegin \DIFadd{_{S,i}}\DIFaddend (c_x)\DIFdelbegin %DIFDELCMD < \ket*{\Psi^{(i)}}
%DIFDELCMD < %%%
...
\begin{equation}\label{approxeigen2}
  \hat{T}_S(x)\DIFdelbegin %DIFDELCMD < \ket*{\Psi^{(i)}}%%%
\DIFdelend \DIFaddbegin \ket*{\Gamma_{i}}\DIFaddend = \tau\DIFdelbegin \DIFdel{_S^{(i)}}\DIFdelend \DIFaddbegin \DIFadd{_{S,i}}\DIFaddend (x)\DIFdelbegin %DIFDELCMD < \ket*{\Psi^{(i)}}
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \ket*{\Gamma_{i}}
  \DIFaddend \end{equation}
...
Now the additional variables beyond standard quantum theory that are included in Kent's theory are given by one of these simultaneous $\hat{T}_S$-eigenvalues \DIFdelbegin \DIFdel{$\tau_S^{(i)}$ }\DIFdelend \DIFaddbegin \DIFadd{$\tau_{S,i}$ }\DIFaddend that (approximately) describe a possible outcome for \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend mass-energy density measurement over the whole of $S$. \DIFdelbegin \DIFdel{But although we speak of the measurement of $T_S(x)$ as being $\tau_S^{(i)}(x)$, this is only a notional measurement. Thus, we speak of the measurement of  $T_S(x)$ only to mean that $T_S(x)$ has a determinate value for every $x\in S$ despite the quantum state of $S$ in general being in a superposition of $\hat{T}_S(x)$-eigenstates for any given $x\in S$. How this determination of $T_S(x)$ comes about is up to one's philosophical preferences. 
For example, one could suppose that it was simply by divine fiat that this determination  of $T_S(x)$ came about.}\footnote{\DIFdel{I will discuss my philosophical preference in the final chapter.}} 
%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{We will let $\tau_S$ denote the particular  $\tau_{S,i}$ that constitute the additional variables of Kent's theory. 
}\DIFaddend 

\DIFdelbegin \DIFdel{Nevertheless, }\DIFdelend \DIFaddbegin \DIFadd{Now }\DIFaddend the particular density \DIFdelbegin \DIFdel{$\tau_S(x)$ }\DIFdelend \DIFaddbegin \DIFadd{$\tau_S$ }\DIFaddend which is found to describe $S$ can't be absolutely anything. \DIFdelbegin \DIFdel{Rather, we suppose there is a much earlier hypersurface $S_0$  %DIF < 
}%DIFDELCMD < \nomenclature{$S_0$}{Initial hypersurface for which Kent assumes all physics occurs between $S_0$ and $S$, \nomrefpage}%%%
%DIF < 
\DIFdel{which is described by a state $\ket*{\Psi_0}$ belonging to a Hilbert space $H_{S_0}$ as shown in figure \ref{S1}. It is assumed }\DIFdelend \DIFaddbegin \DIFadd{Limitations are placed on what $\tau_S$ can be, and these limitations will depend on the initial conditions of Kent's theory. Kent assumes }\DIFaddend that all physics that we wish to describe takes place between \DIFdelbegin \DIFdel{these }\DIFdelend two hypersurfaces $S_0$ %DIF > 
\DIFaddbegin \nomenclature{$S_0$}{Initial hypersurface for which Kent assumes all physics occurs between $S_0$ and $S$, \nomrefpage}%DIF > 
 \DIFaddend and $S$\DIFdelbegin \DIFdel{. In figure \ref{S1}, we therefore let $y$ depicts a generic spacetime location that we wish to describe. }%DIFDELCMD < 
...
%DIFDELCMD < \draw[<->](-\lrange, \a) node[left] {$S$} --  (\rrange, \a)  node[right] {$S$};                   
%DIFDELCMD < \draw[->] (\rrange,\md-\tlen/2) --  (\rrange,\md+\tlen/2) node[midway,right]{time}; 
%DIFDELCMD < \coordinate[label = above: Notional Measurement of $T_S(x)$ on $S$]  (D) at (\fictlabel,\a+0.2); 
...
%DIFDELCMD < \draw [->, shorten <= 5pt] (start) [above] -- (final); 
...
%DIFDELCMD < \vspace*{-12px}
%DIFDELCMD < 

%DIFDELCMD <  %%%
\DIFdel{If we  now define 
}\DIFdelend \DIFaddbegin \DIFadd{, with $S_0$ much earlier than $S$ so that $S_0$ and $S$ don't intersect. Initial conditions are determined on the hypersurface $S_0$ so that we can assume it is described by a state $\ket*{\Psi_0}\in H_{S_0}$ in the Tomonaga-Schwinger picture. %DIF > 
 }\nomenclature{$\ket*{\Psi_0}$}{The state of the initial hypersurface $S_0$ in the Tomonaga-Schwinger picture, \nomrefpage}%DIF >  
 \DIFadd{If we  define 
}\DIFaddend \begin{equation}\label{SchwingerUnitaryOP}
U_{SS_0}=U[S]U[S_0]^{-1}\DIFaddbegin \DIFadd{,
}\DIFaddend \end{equation} 
\DIFdelbegin \DIFdel{then $U_{SS_0}$ will }\DIFdelend \DIFaddbegin \DIFadd{where $U[S]$ and $U[S_0]$ are the Schwinger unitary operators introduced on
 page \pageref{SchwingerOperator}, then
 given the state $\ket*{\Psi_0}$, there will be a corresponding state $\ket*{\Psi_S}=U_{SS_0}\ket*{\Psi_0}\in H_S$  }\DIFaddend %
...
%DIF < 
  \DIFdel{be a unitary operator that maps states in $H_{S_0}$ such as }\DIFdelend \DIFaddbegin \nomenclature{$\ket*{\Psi_S}$}{The state $U_{SS_0}\ket*{\Psi_0}\in H_S$ in the Tomonaga-Schwinger picture, \nomrefpage}%DIF > 
  \DIFadd{that describes the hypersurface $S$ in the Tomonaga-Schwinger picture. Figure \ref{S1} depicts the evolution of the state }\DIFaddend $\ket*{\Psi_0}$ to \DIFdelbegin \DIFdel{states in $H_S$.
  Then the probability $P(\Psi|\Psi_0)$ }\DIFdelend \DIFaddbegin \DIFadd{the state $\ket*{\Psi_S}$.
  }\begin{figure}[ht!]
    \captionsetup{justification=justified}
    \centering
...
\DIFdelend \DIFaddbegin \nomenclature{$P(\Gamma|\Psi_0)$}{The probability that  $S$ will be found to be in the state $\ket*{\Gamma}$ with mass-energy density $\tau_S(x)$ given that $S_0$ was initially in the state $\ket*{\Psi_0}$, see equation (\ref{bornrule}), \nomrefpage}\DIFaddend %
  that $S$ will be found to be in the state \DIFdelbegin \DIFdel{$\ket*{\Psi}$ with mass-energy density $\tau_S(x)$ given that $S_0$ was initially in the }\DIFdelend \DIFaddbegin \DIFadd{$\ket*{\Gamma}$ given the initial }\DIFaddend state $\ket*{\Psi_0}$ \DIFaddbegin \DIFadd{describing $S_0$ }\DIFaddend will be given by the Born Rule (see page \pageref{bornrule}):    
  \begin{equation}\label{bornpsi}
    P(\DIFdelbegin \DIFdel{\Psi}\DIFdelend \DIFaddbegin \DIFadd{\Gamma}\DIFaddend |\Psi_0) = \DIFdelbegin %DIFDELCMD < \abs{\mel{\Psi}{U_{SS_0}}{\Psi_0}}%%%
\DIFdelend \DIFaddbegin \abs{\mel{\Gamma}{U_{SS_0}}{\Psi_0}}\DIFaddend ^2=\DIFdelbegin %DIFDELCMD < \abs{\ip{\Psi}{\Psi_S}}%%%
\DIFdelend \DIFaddbegin \abs{\ip{\Gamma}{\Psi_S}}\DIFaddend ^2
    \end{equation}
  \DIFdelbegin \DIFdel{where $\ket*{\Psi_S}=U_{SS_0}\ket*{\Psi_0}.$
}\DIFdelend It's  %DIF < 
\DIFdelbegin %DIFDELCMD < \nomenclature{$\ket*{\Psi_S}$}{The state $U_{SS_0}\ket*{\Psi_0}\in H_S$ in the Tomonaga-Schwinger picture, \nomrefpage}%%%
%DIF < 
 \DIFdelend possible that there \DIFdelbegin \DIFdel{might be several different states of }\DIFdelend \DIFaddbegin \DIFadd{could be more than one simultaneous  $\hat{T}_S$-eigenstate  in  }\DIFaddend $H_S$ that \DIFdelbegin \DIFdel{have the same mass-energy density $\tau_S(x)$ for all $x\in S$, but one of these states is still realized with probability given by equation (\ref{bornpsi}). But }\DIFdelend \DIFaddbegin \DIFadd{has the simultaneous  $\hat{T}_S$-eigenvalue $\tau_S$, but }\DIFaddend it is the mass-energy density $\tau_S$ itself rather than one of the eigenstates with mass-energy density $\tau_S$ that constitute the additional variables that Kent adds to standard quantum theory. 

  Also note that  if every simultaneous $\hat{T}_S$-eigenstate \DIFdelbegin \DIFdel{$\ket*{\Psi}$ }\DIFdelend \DIFaddbegin \DIFadd{$\ket*{\Gamma}$ }\DIFaddend with simultaneous $\hat{T}_S$-eigenvalue $\tau_S$ satisfies \DIFdelbegin \DIFdel{$\abs{\mel{\Psi}{U_{SS_0}}{\Psi_0}}=0$}\DIFdelend \DIFaddbegin \DIFadd{$\abs{\mel{\Gamma}{U_{SS_0}}{\Psi_0}}=0$}\DIFaddend , then by (\ref{bornpsi}), $\tau_S$ will \DIFaddbegin \DIFadd{have zero probability, and hence, it will }\DIFaddend not be a possible measurement outcome for $T_S$ given $\ket*{\Psi_0}$.\DIFaddbegin \footnote{\label{discreteRV}\DIFadd{When dealing with continuous random variables, if the probability of the random variable having a particular value is  zero, it does not follow that it is impossible for the random variable to have this value. However, in the case of discrete random variables,  if the probability of the random variable having a particular value is  zero, then it does  follow that it is impossible for the random variable to have this value. Because we are using (\ref{tauapproxformula}) to approximate the energy density, we can treat  $T_S$ as a discrete random variable, hence the claim that if $T_S=\tau_S$ has zero probability,  then $\tau_S$ will not be a possible measurement outcome for $T_S$ given $\ket*{\Psi_0}$.}} \DIFaddend It is for this reason that we can't expect the measurement outcome of $T_S$ on $S$ to be absolutely anything.
...
\section{The One-World Feature of Kent's Theory\DIFaddbegin \label{OneWorldFeature}\DIFaddend }
The third similarity Kent's theory shares with the \DIFdelbegin \DIFdel{pilot wave }\DIFdelend \DIFaddbegin \DIFadd{Bohmian }\DIFaddend interpretation is that it is a one-world interpretation of quantum physics. It will be helpful to contrast this with the many-worlds interpretation. 
...
Now the beables in Kent's \DIFdelbegin \DIFdel{one world }\DIFdelend \DIFaddbegin \DIFadd{one-world }\DIFaddend interpretation are expressed in terms of a physical quantity called the \textbf{stress-energy tensor}\index{stress-energy tensor}  $T^{\mu\nu}(y)$.\label{stressenergy}   %
...
\vspace*{2px}
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Depiction of $S^1(y)$]{\DIFaddendFL The set $S^1(y)$ consists of all the spacetime locations of $S$ outside the light cone of $y$. The $T^{\mu\nu}(y)$-beables are calculated using the initial state $\ket*{\Psi_0}$ together with the values of $\tau_S(x)$ for $x\in S^1(y)$. }
\label{S2}
\end{figure}
...
\DIFdelendFL \DIFaddbeginFL \caption[Depiction of Schr\"{o}dinger's living cat]{\DIFaddendFL A depiction of Schr\"{o}dinger's cat being alive.\protect\footnotemark}
  ...
  \begin{figure}[ht!]
    ...
    \DIFdelbeginFL %DIFDELCMD < \caption[Caption for LOF]{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Depiction of Schr\"{o}dinger's dead cat]{\DIFaddendFL A depiction of Schr\"{o}dinger's cat being dead.\protect\footnotemark}
    \label{deadcat}
    \end{figure}
...
The recipe for defining Kent's $T^{\mu\nu}(y)$-beable is first to select\DIFaddbegin \footnote{\DIFadd{See pages \pageref{selectionmeaningbeg} to \pageref{selectionmeaningend} for a discussion of what is meant by selection.}} \DIFaddend an outcome $\tau_S$ that is defined over all of $S$. The outcome $\tau_S$ is selected with probability determined by the Born rule using equation (\ref{bornpsi}). \DIFdelbegin \footnote{\DIFdel{If there is only one state $\ket*{\Psi}$ such that $\hat{T}_S(x)\ket*{\Psi}=\tau_S(x)\ket*{\Psi}$ for all $x\in S$, then the probability $P(\tau_S)$ that $\tau_S$ is selected will be precisely the probability given by equation (\ref{bornpsi}). But if there are several states $\{\ket*{\Psi_\alpha}:\alpha\}$ such that  $\hat{T}_S(x)\ket*{\Psi_\alpha}=\tau_S(x)\ket*{\Psi_\alpha}$, then the probability $P(\tau_S)$ that $\tau_S$ is selected will be 
}\begin{displaymath}\DIFdel{P(\tau_S)=\sum_\alpha \abs{\mel{\Psi_\alpha}{U_{SS_0}}{\Psi_0}}^2=\sum_\alpha \abs{\ip{\Psi_\alpha}{\Psi_S}}^2.}\end{displaymath}%DIFAUXCMD
%DIFDELCMD <  
%DIFDELCMD < %%%
\DIFdel{Also note that while no one outcome for $\tau_S$ is going to be very likely, having the outcome $\tau_S$ shouldn't be highly improbable relative to other possible mass-energy density outcomes. That is, if $P(\tau_S)\ll P(\tau_S')$, then $\tau_S$ shouldn't be selected.}} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend Then Kent's $T^{\mu\nu}(y)$-beable for any $y$ between $S_0$ and $S$ is defined to be
...
We can see how the formula (\ref{conditionalexpectation}) relates to the \DIFdelbegin \DIFdel{Sch}\DIFdelend \DIFaddbegin \DIFadd{Schr}\DIFaddend \"{o}dinger's cat scenario. The distribution of light reflected off the cat that intersects $S^1(y)$ when ``measured'' will determine a definite statement $r(\tau_S, y)$ about the mass-energy density on $S^1(y)$. This in turn will determine the range of $\tau$ for which $P(q(\tau)|r(\tau_S,y))$ is not close to zero, and hence where the stress-energy distribution $\ev*{T^{\mu\nu}(y)}_{\tau_S}$ is not zero. This stress-energy distribution will then correspond either to that of  a living cat or to that of a dead cat, but not both.  
 ...
 P(r(\tau_S,y))=\sum\DIFdelbegin \DIFdel{_{\substack{i \text{ such that} \\ \tau_S^{(i)}(x)=\tau_S(x)\\ \text{ for all }x\in S^1(y)}}}%DIFDELCMD < \abs{\mel{\Psi^{(i)}}{U_{SS_0}}{\Psi_0}}%%%
...
This means that we can't define $\hat{T}^{\mu\nu}(y)$ according to (\ref{tsobservable}) since $y\not\in S$.\footnote{If we did attempt to use (\ref{tsobservable}) to define $\hat{T}^{\mu\nu}(y)=U[S]\hat{\bm{T}}^{{\mu\nu}}(y)U[S]^{-1}$, then $\hat{T}^{\mu\nu}(y)$ would have a (non-local) dependence on $S$, and such a dependence would not be desirable.} However, we do not face such restrictions in the Heisenberg picture, so one approach would be to calculate $P(q(\tau) \, \&\,  r(\tau_S,y))$ in the Heisenberg picture. As we will see shortly, this is not the approach that Kent takes, but nevertheless, in the Heisenberg picture, it is easier to see why we don't include information from $S$ within the light cone (without begging the question of why we don't) when calculating $P(q(\tau) \, \&\,  r(\tau_S,y))$. To see why this is so, consider the simpler case of just two measurable quantities ${F}$ and ${G}$ which we assume to have a discrete range of possible values and for which we wish to calculate the joint probability $P((F=f)\, \&\, (G=g))$. To do this in the Heisenberg picture, we need an orthonormal basis of the state space \DIFdelbegin \DIFdel{$\{\ket*{\Phi^{(i)}}:i\}$ }\DIFdelend \DIFaddbegin \DIFadd{$\{\ket*{\Phi_{i}}:i\in\mathbb{N}\}$ }\DIFaddend consisting of simultaneous \DIFdelbegin \DIFdel{eigenstate }\DIFdelend \DIFaddbegin \DIFadd{eigenstates }\DIFaddend of the observables $\hat{\bm{F}}$ and $\hat{\bm{G}}$ with eigenvalues \DIFdelbegin \DIFdel{$f^{(i)}$ and $g^{(i)}$ }\DIFdelend \DIFaddbegin \DIFadd{$f_{i}$ and $g_{i}$ }\DIFaddend respectively so that \DIFdelbegin \DIFdel{$\hat{\bm{F}}\ket*{\Phi^{(i)}}=f^{(i)}\ket*{\Phi^{(i)}}$ and $\hat{\bm{G}}\ket*{\Phi^{(i)}}=g^{(i)}\ket*{\Phi^{(i)}}$}\DIFdelend \DIFaddbegin \DIFadd{$\hat{\bm{F}}\ket*{\Phi_{i}}=f_{i}\ket*{\Phi_{i}}$ and $\hat{\bm{G}}\ket*{\Phi_{i}}=g_{i}\ket*{\Phi_{i}}$}\DIFaddend , and when the system is in the state \DIFdelbegin \DIFdel{$\ket*{\Phi^{(i)}}$}\DIFdelend \DIFaddbegin \DIFadd{$\ket*{\Phi_{i}}$}\DIFaddend , the quantity $F$ will have the value \DIFdelbegin \DIFdel{$f^{(i)}$}\DIFdelend \DIFaddbegin \DIFadd{$f_{i}$}\DIFaddend , and the quantity $G$ will have the value \DIFdelbegin \DIFdel{$g^{(i)}$}\DIFdelend \DIFaddbegin \DIFadd{$g_{i}$}\DIFaddend . Given that the system is in the state $\ket*{\Phi}$, the joint probability $P((F=f)\, \&\, (G=g))$ can then be calculated using the Born rule to get
$$P((F=f)\, \&\, (G=g))=\sum\DIFdelbegin \DIFdel{_{\substack{i \text{ such that} \\ f^{(i)}=f \text{ and } g^{(i)}=g}}}%DIFDELCMD < \abs{\ip{\Phi^{(i)}}{\Phi}}%%%
...
\DIFdelbegin \begin{displaymath}\DIFdel{\hat{\bm{F}}\hat{\bm{G}}\ket*{\Phi^{(i)}}=f^{(i)}g^{(i)}\ket*{\Phi^{(i)}}=g^{(i)}f^{(i)}\ket*{\Phi^{(i)}}=\hat{\bm{G}}\hat{\bm{F}}\ket*{\Phi^{(i)}}}\end{displaymath}%DIFAUXCMD
...
\DIFdelbegin \begin{displaymath}\DIFdel{\hat{\bm{F}}\hat{\bm{G}}\ket*{\Phi}=\sum_i c_i \hat{\bm{F}}\hat{\bm{G}}\ket*{\Phi^{(i)}}= \sum_i c_i \hat{\bm{G}}\hat{\bm{F}}\ket*{\Phi^{(i)}}=\hat{\bm{G}}\hat{\bm{F}}\ket*{\Phi}.}\end{displaymath}%DIFAUXCMD
...
\vspace*{2px}
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Hypersurface $S_n\myeq S_n(y)$ containing $y$]{\DIFaddendFL $S_n\myeq S_n(y)$ is a hypersurface containing $y$ and all of $S^1(y)$ in the limit as $n\rightarrow\infty$.   }
\label{S3}
...
\vspace*{2px}
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Photon measurements for energy density calculations at $y^a_1$ and $y^a_2$]{\DIFaddendFL (a) highlights the part of $S$ used to calculate the energy density at $y^a_1$ whose time is less than $2t_1-t_2$. (b) highlights the part of $S$ used to calculate the energy density at $y^a_2$ whose time is less than $2t_1-t_2$.}
\label{TM2}
\end{figure}
...
\vspace*{2px}
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Photon measurements for energy density calculations at $y^b_1$ and $y^b_2$]{\DIFaddendFL (a) highlights the part of $S$ used to calculate the energy density at $y^b_1$ whose time is greater than $2t_1-t_2$. (b) highlights the part of $S$ used to calculate the energy density at $y^b_2$ whose time is greater than $2t_1-t_2$.}
\label{TM3}
\end{figure}
...
\DIFdelend %DIF > \usepackage[inline]{showlabels}
\DIFaddbegin \newpage 
\DIFaddend % flatex input: [Chapter04/chapterIntro.tex]
\DIFaddbegin 
...
\pi_n=\sum_j\DIFdelbegin %DIFDELCMD < \dyad{\xi_j}%%%
...
\DIFdelend \DIFaddbegin \nomenclature{$\{\ket*{\eta_j^{\mu\nu}}:j\in\mathbb{N}\}$}{An orthonormal basis of $H_{S_n}$ consisting of simultaneous $\hat{T}^{\mu\nu}(y)$, $\hat{T}_S(x)$-eigenstates so that $\hat{T}^{\mu\nu}(y)\ket*{\eta_j^{\mu\nu}}=\tau_{j}\ket*{\eta_j^{\mu\nu}}$ and $\hat{T}_{S}(x)\ket*{\eta_j^{\mu\nu}}=\tau_{S,j}(x)\ket*{\eta_j^{\mu\nu}}$ for $x\in S_n\cap S$, where $\tau_{j}$ and $\tau_{S,j}(x)$ are the corresponding eigenvalues, \nomrefpage}\DIFaddend %
 $H_{S_n}$ consisting of simultaneous $\hat{T}^{\mu\nu}(y)$, $\hat{T}_S(x)$-eigenstates so that \DIFdelbegin \DIFdel{$\hat{T}^{\mu\nu}(y)\ket*{\eta_j}=\tau^{(j)}\ket*{\eta_j}$ and $\hat{T}_{S}(x)\ket*{\eta_j}=\tau_S^{(j)}(x)\ket*{\eta_j}$ }\DIFdelend \DIFaddbegin \DIFadd{$\hat{T}^{\mu\nu}(y)\ket*{\eta_j^{\mu\nu}}=\tau_{j}\ket*{\eta_j^{\mu\nu}}$ and $\hat{T}_{S}(x)\ket*{\eta_j^{\mu\nu}}=\tau_{S,j}(x)\ket*{\eta_j^{\mu\nu}}$ }\DIFaddend for $x\in S_n\cap S$, where \DIFdelbegin \DIFdel{$\tau^{(j)}$ and $\tau_S^{(j)}(x)$ }\DIFdelend \DIFaddbegin \DIFadd{$\tau_{j}$ and $\tau_{S,j}(x)$ }\DIFaddend are the corresponding eigenvalues.\DIFaddbegin \footnote{\DIFadd{We include the indices   $\mu,\nu$ in the specification of the basis $\{\ket*{\eta_j^{\mu\nu}}:j\in\mathbb{N}\}$ since in general, we can't assume that $\hat{T}^{\mu\nu}(y)$ and $\hat{T}^{\mu'\nu'}(y)$ commute for $\mu\neq \mu'$ or $\nu\neq \nu'$, and so we have to fix $\mu,\nu$ and consider a basis $\{\ket*{\eta_j^{\mu\nu}}:j\in\mathbb{N}\}$ of $H_n$ which depends on the particular $\mu,\nu$ so that all the elements of this basis can be eigenstates of  $\hat{T}^{\mu\nu}(y)$.}} \DIFaddend If we define %
...
\DIFdelend \DIFaddbegin \nomenclature{$\pi_{n,\tau}^{\mu\nu}$}{The projection $\pi_{n,\tau}^{\mu\nu}=\sum_j\dyad*{\chi_{j,\tau}^{\mu\nu}}$, \nomrefpage}\DIFaddend %
   \DIFdelbegin \DIFdel{$\pi_{n,\tau}=\sum_j\dyad{\chi_{j,\tau}}$ where $\{\ket*{\chi_{j,\tau}}:j\}$ }\DIFdelend \DIFaddbegin \DIFadd{$\pi_{n,\tau}^{\mu\nu}=\sum_j\dyad*{\chi_{j,\tau}^{\mu\nu}}$ where $\{\ket*{\chi_{j,\tau}^{\mu\nu}}:j\in\mathbb{N}\}$ }\DIFaddend %
...
\DIFdelend \DIFaddbegin \nomenclature{$\{\ket*{\chi_{j,\tau}^{\mu\nu}}:j\in\mathbb{N}\}$}{The subset of $\{\ket*{\eta_j^{\mu\nu}}:j\in\mathbb{N}\}$ such that $\hat{T}^{\mu\nu}(y)\ket*{\chi_{j,\tau}^{\mu\nu}}=\tau\ket*{\chi_{j,\tau}^{\mu\nu}}$ and $\hat{T}_S(x)\ket*{\chi_{j,\tau}^{\mu\nu}}=\tau_S(x)\ket*{\chi_{j,\tau}^{\mu\nu}}$ for all $x\in S_n\cap S$, \nomrefpage}\DIFaddend %
  is the subset of \DIFdelbegin \DIFdel{$\{\ket*{\eta_j}:j\}$ such that $\hat{T}^{\mu\nu}(y)\ket*{\chi_{j,\tau}}=\tau\ket*{\chi_{j,\tau}}$ and $\hat{T}_S(x)\ket*{\chi_{j,\tau}}=\tau_S(x)\ket*{\chi_{j,\tau}}$ }\DIFdelend \DIFaddbegin \DIFadd{$\{\ket*{\eta_j^{\mu\nu}}:j\in\mathbb{N}\}$ such that $\hat{T}^{\mu\nu}(y)\ket*{\chi_{j,\tau}^{\mu\nu}}=\tau\ket*{\chi_{j,\tau}^{\mu\nu}}$ and $\hat{T}_S(x)\ket*{\chi_{j,\tau}^{\mu\nu}}=\tau_S(x)\ket*{\chi_{j,\tau}^{\mu\nu}}$ }\DIFaddend for all $x\in S_n\cap S$, then 
\begin{equation}\label{pqtauri}
P(q(\tau)\, \& \,r_n)=\sum_j \DIFdelbegin %DIFDELCMD < \abs{ \ip{\chi_{j,\tau}}{\Psi_n}}%%%
\DIFdelend \DIFaddbegin \abs{ \ip{\chi_{j,\tau}^{\mu\nu}}{\Psi_n}}\DIFaddend ^2=\DIFdelbegin %DIFDELCMD < \ev*{\pi_{n,\tau}}{\Psi_n}%%%
\DIFdelend \DIFaddbegin \ev*{\pi_{n,\tau}^{\mu\nu}}{\Psi_n}\DIFaddend .\protect\footnotemark
\end{equation}
...
\DIFdelend \DIFaddbegin \nomenclature{$\pi_\tau^{\mu\nu}$}{The projection $\pi_\tau^{\mu\nu}=\sum_j\dyad*{\eta_{j,\tau}^{\mu\nu}}$, \nomrefpage}\DIFaddend %
 if we define \DIFdelbegin \DIFdel{$\pi_\tau=\sum_j\dyad{\eta_{j,\tau}}$ where $\{\ket*{\eta_{j,\tau}}:j\}$ }\DIFdelend \DIFaddbegin \DIFadd{$\pi_\tau^{\mu\nu}=\sum_j\dyad*{\eta_{j,\tau}^{\mu\nu}}$ where $\{\ket*{\eta_{j,\tau}^{\mu\nu}}:j\in\mathbb{N}\}$ }\DIFaddend is %
...
\DIFdelend \DIFaddbegin \nomenclature{$\{\ket*{\eta_{j,\tau}^{\mu\nu}}:j\in\mathbb{N}\}$}{The subset of  $\{\ket*{\eta_j^{\mu\nu}}:j\in\mathbb{N}\}$ with $\hat{T}^{\mu\nu}(y)\ket*{\eta_{j,\tau}^{\mu\nu}}=\tau\ket*{\eta_{j,\tau}^{\mu\nu}}$, \nomrefpage}\DIFaddend %
  the subset of  \DIFdelbegin \DIFdel{$\{\ket*{\eta_j}:j\}$ with $\hat{T}^{\mu\nu}(y)\ket*{\eta_{j,\tau}}=\tau\ket*{\eta_{j,\tau}}$}\DIFdelend \DIFaddbegin \DIFadd{$\{\ket*{\eta_j^{\mu\nu}}:j\in\mathbb{N}\}$ with $\hat{T}^{\mu\nu}(y)\ket*{\eta_{j,\tau}^{\mu\nu}}=\tau\ket*{\eta_{j,\tau}^{\mu\nu}}$}\DIFaddend , then we also have  \DIFdelbegin \DIFdel{$\pi_{n,\tau}=\pi_n\pi_\tau$.}\DIFdelend \DIFaddbegin \DIFadd{$\pi_{n,\tau}^{\mu\nu}=\pi_n\pi_\tau^{\mu\nu}$.}\DIFaddend \footnote{To see why this is, 
we first show that \DIFdelbegin \DIFdel{$\pi_n=\sum_j\dyad{h_{n,j}}$ }\DIFdelend \DIFaddbegin \DIFadd{$\pi_n=\sum_j\dyad*{h_{n,j}^{\mu\nu}}$ }\DIFaddend where \DIFdelbegin \DIFdel{$\{\ket*{h_{n,j}}:j\}$ }\DIFdelend \DIFaddbegin \DIFadd{$\{\ket*{h_{n,j}^{\mu\nu}}:j\in\mathbb{N}\}$ }\DIFaddend is the subset of \DIFdelbegin \DIFdel{$\{\ket*{\eta_j}:j\}$  }\DIFdelend \DIFaddbegin \DIFadd{$\{\ket*{\eta_j^{\mu\nu}}:j\in\mathbb{N}\}$  }\DIFaddend for which \DIFdelbegin \DIFdel{$\ket*{h_{n,j}}\in H_{S_n,\tau_S}$}\DIFdelend \DIFaddbegin \DIFadd{$\ket*{h_{n,j}^{\mu\nu}}\in H_{S_n,\tau_S}$}\DIFaddend . 
Note that \DIFdelbegin \DIFdel{$\pi_n\ket*{h_{n,j}}=\ket*{h_{n,j}}$    }\DIFdelend \DIFaddbegin \DIFadd{$\pi_n\ket*{h_{n,j}^{\mu\nu}}=\ket*{h_{n,j}^{\mu\nu}}$    }\DIFaddend since \DIFdelbegin \DIFdel{$\{\ket*{\xi_j}:j\}$ }\DIFdelend \DIFaddbegin \DIFadd{$\{\ket*{\xi_j}:j\in\mathbb{N}\}$ }\DIFaddend is a basis for $H_{S_n,\tau_S}$ and \DIFdelbegin \DIFdel{$\ket*{h_{n,j}}\in H_{S_n,\tau_S}$}\DIFdelend \DIFaddbegin \DIFadd{$\ket*{h_{n,j}^{\mu\nu}}\in H_{S_n,\tau_S}$}\DIFaddend . 
Therefore, \DIFdelbegin \DIFdel{$\pi_n\pi_{n,h}=\pi_{n,h}$  }\DIFdelend \DIFaddbegin \DIFadd{$\pi_n\pi_{n,h}^{\mu\nu}=\pi_{n,h}^{\mu\nu}$  }\DIFaddend where  \DIFdelbegin \DIFdel{$\pi_{n,h}=\sum_j\dyad{h_{n,j}}$}\DIFdelend \DIFaddbegin \DIFadd{$\pi_{n,h}^{\mu\nu}=\sum_j\dyad*{h_{n,j}^{\mu\nu}}$}\DIFaddend . 
But  \DIFdelbegin \DIFdel{$\pi_{n,h}\ket*{\xi_j}=\ket*{\xi_j}$ }\DIFdelend \DIFaddbegin \DIFadd{$\pi_{n,h}^{\mu\nu}\ket*{\xi_j}=\ket*{\xi_j}$ }\DIFaddend since \DIFdelbegin \DIFdel{$\{\ket*{h_{n,j}}:j\}$ }\DIFdelend \DIFaddbegin \DIFadd{$\{\ket*{h_{n,j}^{\mu\nu}}:j\in\mathbb{N}\}$ }\DIFaddend is a basis for $H_{S_n,\tau_S}$ and $\ket*{\xi_j}\in H_{S_n,\tau_S}$. 
Therefore, \DIFdelbegin \DIFdel{$\pi_{n,h}\pi_n=\pi_n.$ }\DIFdelend \DIFaddbegin \DIFadd{$\pi_{n,h}^{\mu\nu}\pi_n=\pi_n.$ }\DIFaddend But \DIFdelbegin \DIFdel{$\pi_{n,h}\pi_n= \pi_n\pi_{n,h}$ }\DIFdelend \DIFaddbegin \DIFadd{$\pi_{n,h}^{\mu\nu}\pi_n= \pi_n\pi_{n,h}^{\mu\nu}$ }\DIFaddend since $\pi_n$ and \DIFdelbegin \DIFdel{$\pi_{n,h}$ }\DIFdelend \DIFaddbegin \DIFadd{$\pi_{n,h}^{\mu\nu}$ }\DIFaddend are Hermitian\DIFaddbegin \DIFadd{/self-adjoint}\DIFaddend . Hence, \DIFdelbegin \DIFdel{$\pi_n= \pi_{n,h}$}\DIFdelend \DIFaddbegin \DIFadd{$\pi_n= \pi_{n,h}^{\mu\nu}$}\DIFaddend . Now the summands of \DIFdelbegin \DIFdel{$\pi_n\pi_\tau$ }\DIFdelend \DIFaddbegin \DIFadd{$\pi_n\pi_\tau^{\mu\nu}$ }\DIFaddend are only going to consist of those \DIFdelbegin \DIFdel{$\dyad{\eta_j}$ }\DIFdelend \DIFaddbegin \DIFadd{$\dyad*{\eta_j^{\mu\nu}}$ }\DIFaddend for which \DIFdelbegin \DIFdel{$\hat{T}^{\mu\nu}(y)\ket*{\eta_j}=\tau\ket*{\eta_j}$ }\DIFdelend \DIFaddbegin \DIFadd{$\hat{T}^{\mu\nu}(y)\ket*{\eta_j^{\mu\nu}}=\tau\ket*{\eta_j^{\mu\nu}}$ }\DIFaddend and for which \DIFdelbegin \DIFdel{$\hat{T}_S(x)\ket*{\eta_j}=\tau_S(x)\ket*{\eta_j}$ }\DIFdelend \DIFaddbegin \DIFadd{$\hat{T}_S(x)\ket*{\eta_j^{\mu\nu}}=\tau_S(x)\ket*{\eta_j^{\mu\nu}}$ }\DIFaddend for all $x\in S_n\cap S$, and these are just the \DIFdelbegin \DIFdel{$\dyad*{\chi_{j,\tau}}$ }\DIFdelend \DIFaddbegin \DIFadd{$\dyad*{\chi_{j,\tau}^{\mu\nu}}$ }\DIFaddend which are the summands of  \DIFdelbegin \DIFdel{$\pi_{n,\tau}$}\DIFdelend \DIFaddbegin \DIFadd{$\pi_{n,\tau}^{\mu\nu}$}\DIFaddend . Hence,  \DIFdelbegin \DIFdel{$\pi_n\pi_\tau=\pi_{n,\tau}.$}\DIFdelend \DIFaddbegin \DIFadd{$\pi_n\pi_\tau^{\mu\nu}=\pi_{n,\tau}^{\mu\nu}.$}\DIFaddend } 
  Hence,
\begin{equation}\label{pqtauri2}
P(q(\tau)\, \& \, r_n)=\DIFdelbegin %DIFDELCMD < \ev*{\pi_n\pi_\tau}{\Psi_n}%%%
\DIFdelend \DIFaddbegin \ev*{\pi_n\pi_\tau^{\mu\nu}}{\Psi_n}\DIFaddend .
\end{equation}
But clearly \DIFdelbegin \DIFdel{$\hat{T}^{\mu\nu}(y)=\sum_\tau \tau \pi_\tau.$ }\DIFdelend \DIFaddbegin \DIFadd{$\hat{T}^{\mu\nu}(y)=\sum_\tau \tau \pi_\tau^{\mu\nu}.$ }\DIFaddend Therefore, combining (\ref{beable1}), (\ref{Prn}), and (\ref{pqtauri2}), we have 
\begin{equation}\label{kentconsistency0}
\ev*{T^{\mu\nu}(y)}_{\tau_S}=\lim_{n\rightarrow\infty}\DIFdelbegin \DIFdel{\frac{\sum_\tau \ev*{\pi_n\pi_\tau}{\Psi_n}\tau}{\ev*{\pi_n}{\Psi_n}}}\DIFdelend \DIFaddbegin \DIFadd{\frac{\sum_\tau \ev*{\pi_n\pi_\tau^{\mu\nu}}{\Psi_n}\tau}{\ev*{\pi_n}{\Psi_n}}}\DIFaddend =\lim_{n\rightarrow\infty}\frac{\ev*{\pi_n\hat{T}^{\mu\nu}(y)}{\Psi_n}}{\ev*{\pi_n}{\Psi_n}}.
...
\lim_{n\rightarrow\infty}\sum_jP\big(r_n(\tau\DIFdelbegin \DIFdel{^{(j)}_{S_n\cap S}}\DIFdelend \DIFaddbegin \DIFadd{_{S_n\cap S, j}}\DIFaddend )\big)\ev*{T^{\mu\nu}(y)}\DIFdelbegin \DIFdel{_{\tau^{(j)}_{S_n\cap S}}}\DIFdelend \DIFaddbegin \DIFadd{_{\tau_{S_n\cap S, j}}}\DIFaddend =\lim_{n\rightarrow\infty}\sum\DIFdelbegin \DIFdel{_{j}{\ev*{\pi_{n}(\tau^{(j)}_{S_n\cap S})\hat{T}^{\mu\nu}(y)}{\Psi_n}}
}\DIFdelend \DIFaddbegin \DIFadd{_{j}{\ev*{\pi_{n}(\tau_{S_n\cap S, j})\hat{T}^{\mu\nu}(y)}{\Psi_n}}
}\DIFaddend \end{equation}
Since there is an orthonormal basis \DIFdelbegin \DIFdel{$\{\ket*{\eta_j}:j\}$ }\DIFdelend \DIFaddbegin \DIFadd{$\{\ket*{\eta_j^{\mu\nu}}:j\in\mathbb{N}\}$ }\DIFaddend of $H_{S_n}$ consisting of simultaneous $\hat{T}_S(x)$-eigenstates so that \DIFdelbegin \DIFdel{$\hat{T}_S(x)\ket*{\eta_j}=\tau^{(j)}_{S_n\cap S}(x)\ket*{\eta_j}$ }\DIFdelend \DIFaddbegin \DIFadd{$\hat{T}_S(x)\ket*{\eta_j^{\mu\nu}}=\tau_{S_n\cap S, j}(x)\ket*{\eta_j^{\mu\nu}}$ }\DIFaddend for all $x\in S_n\cap S$, it follows that \DIFdelbegin \DIFdel{$\sum_j \pi_{n}(\tau^{(j)}_{S_n\cap S})=I$}\DIFdelend \DIFaddbegin \DIFadd{$\sum_j \pi_{n}(\tau_{S_n\cap S, j})=I$, where $I$ is the identity operator on $H_n$}\DIFaddend . Combining this with (\ref{kentconsistency1}) we get
\begin{equation}\label{kentconsistency2}
	\lim_{n\rightarrow\infty}\sum_jP\big(r_n(\tau\DIFdelbegin \DIFdel{^{(j)}_{S_n\cap S}}\DIFdelend \DIFaddbegin \DIFadd{_{S_n\cap S, j}}\DIFaddend )\big)\ev*{T^{\mu\nu}(y)}\DIFdelbegin \DIFdel{_{\tau^{(j)}_{S_n\cap S}}}\DIFdelend \DIFaddbegin \DIFadd{_{\tau_{S_n\cap S, j}}}\DIFaddend =\lim_{n\rightarrow\infty}\ev*{\hat{T}^{\mu\nu}(y)}{\Psi_n}
	\end{equation}
...
\vspace*{2px}
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Coordinate transformation]{\DIFaddFL{Coordinate transformation. }\DIFaddendFL Shows how a location (marked as $\bullet$) can be expressed either  in coordinates $(x^1, x^2)$ with respect to the basis $\{\hat{e}_1,\hat{e}_2\}$ or in coordinates $({x'}^1,{x'}^2)$ with respect to the basis $\{\hat{e}'_1,\hat{e}'_2\}$.}\label{rotfigure}
\end{figure}
...
\begin{figure}[ht!]
	...
	\vspace*{2px}
	\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Four-vector transformation]{\DIFaddendFL Shows how a four-vector ${\varphi}$ (of which only two components are shown) defined at a spacetime location (indicated by $\bullet$) can be expressed either as $(\varphi^1, \varphi^2)$ with respect to the basis $\{\hat{e}_1,\hat{e}_2\}$ or as $({\varphi'}^1,{\varphi'}^2)$ with respect to the basis $\{\hat{e}'_1,\hat{e}'_2\}$.}\label{rotfigure2}
	\end{figure}
...
}\DIFaddend $$\ket*{\psi'}=U(\Lambda)\ket*{\psi}.\protect\footnotemark$$\footnotetext{For more details, see \cite{Wigner1939}. Since a unitary operator maps a Hilbert space to itself, we first need to identify $H_{S_n}$ and $H_{S_n}'$ in order for $U(\Lambda)$ to be unitary.}At this point, it is worth clarifying the different meanings of $T^{\mu\nu}(x)$, $T^{\prime\mu\nu}(x')$, $\tau^{\mu\nu}(x)$, $\tau^{\prime\mu\nu}(x')$,  $\hat{T}^{\mu\nu}(x)$ and $\hat{T}^{\prime\mu\nu}(x')$. 
\begin{itemize}
...
\hat{T}^{\prime\mu\nu}(x')\ket*{\psi'}&\approx{\tau'}^{\mu\nu}(x')\ket*{\psi'}.\DIFdelbegin %DIFDELCMD < \label{stres1}
%DIFDELCMD < %%%
...
\vspace*{2px}
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Depiction of localized states prior to any photon interaction]{\DIFaddendFL Depiction of a superposition of two local states at $z_1$ and $z_2$ before the photon has interacted with them. The \DIFaddbeginFL \DIFaddFL{small }\DIFaddendFL gray squares indicate cells in $S^1(y^a_1)$ whose states are among the summands in (\ref{Sistate}) rather than in $\Xi$. The white square indicates a cell in $S_n(y^a_1)$ whose state is a factor in $\Xi$.}
\label{kentdeco1}
\end{figure}

The \DIFaddbegin \DIFadd{small }\DIFaddend gray squares correspond to the summands that appear in (\ref{Sistate}). If the system were in the $\psi_1^\text{sys}$-state, then the state describing $S_n(y^a_1)$ would have a factor $\ket*{\psi_1^\text{sys}}\in H_1$ indicating that there is a 
...
There is also an incoming photon at the $y^a_3$-cell, and so we use $\ket*{\gamma_3}$ to indicate that there is a photon there.
 ...
 as $\ket*{\Psi_n}=\ket*{\psi_1^\text{sys}}\ket*{0_2}\ket*{\gamma_3}\ket*{\Xi'}$, where $\ket*{\Xi'}$ describes the states of all the other cells of $S_n(y^a_1)$. In this very simple scenario, \DIFaddbegin \DIFadd{we set }\DIFaddend $\ket*{\Xi'}=\prod_{k\neq 1,2,3}\ket*{0_k}$ \DIFdelbegin \DIFdel{indicating }\DIFdelend \DIFaddbegin \DIFadd{to indicate }\DIFaddend that there is zero mass/energy at all the other $y_k$ \DIFaddbegin \DIFadd{cells belonging to $S_n(y^a_1)$}\DIFaddend .
...
\vspace*{2px}
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Depiction of localized states after photon interaction]{\DIFaddendFL Depiction of a superposition of two local states at $z_1$ and $z_2$ with $S_n(y^b_1)$ being after the photon has interacted without the photon intersecting $S_n(y^b_1)\cap S$. The \DIFaddbeginFL \DIFaddFL{small }\DIFaddendFL gray squares indicate cells in $S^1(y^b_1)$ whose states are among the summands in (\ref{Sistate}).}
\label{kentdecoh2}
...
\vspace*{2px}
\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Depiction of localized states with reflected photons intersecting $S_n(y^c_1)$]{\DIFaddendFL Depiction of a superposition of two local states at $z_1$ and $z_2$ with $y^c_1$ sufficiently late that the photon intersects $S_n(y^c_1)\cap S$. The \DIFaddbeginFL \DIFaddFL{small }\DIFaddendFL gray squares indicate cells in $S^1(y^c_1)$ whose states are among the summands in (\ref{Sistate})}
\label{kentdecoh3}
\end{figure}
...
\end{enumerate} 
This criterion is satisfied in the \DIFdelbegin \DIFdel{pilot wave }\DIFdelend \DIFaddbegin \DIFadd{Bohmian }\DIFaddend interpretation, since the quantum state is the pilot wave itself. The pilot wave could be other than it is without any of the positions and momenta of the particles changing, but changing the pilot wave would result in a physical change of the system since the pilot wave governs how  the positions and the momenta of the particles subsequently evolve over time.\DIFaddbegin \footnote{\DIFadd{It's worth noting that in the Bohmian interpretation, while the hidden variables of a system can change without the quantum state changing and vice versa, the quantum state in the Bohmian interpretation still places constraints on the hidden variables. For example, as explained in \mbox{%DIFAUXCMD
\cite[31]{DurrDetlef2013Qpwq}}\hskip0pt%DIFAUXCMD
, for a system of $N$ particles with the $k$\textsuperscript{th} particle located at $\vb{x}_k$, we have
}\begin{equation}\DIFadd{\label{BohmVel}\dv{\vb{x}_k}{t} = \frac{\hbar}{m_k} \Im \frac{\grad_k\psi}{\psi}(\vb{x}_1,\ldots,\vb{x}_N, t),
}\end{equation}
\DIFadd{where $\psi(\vb{x}_1,\ldots,\vb{x}_N, t)$ is the quantum wave function for the system, $t$ is time, $m_k$ is the mass of the $k$\textsuperscript{th} particle, and $\grad_k$ is the gradient corresponding to the position variable $\vb{x}_k$. Because of the $\psi$ term in the denominator
 ...
 \DIFaddend We will discuss the failure of criterion \ref{hidden3} for Kent's theory at the end of this section.
...
\end{equation}
for any measurement $O_\mathcal{S}$ on $\mathcal{S}$ and $O_\mathcal{A}$ on $\mathcal{A}$\DIFdelbegin \DIFdel{.}\DIFdelend \DIFaddbegin \DIFadd{, and for almost all $\lambda$.}\DIFaddend \footnote{\DIFdelbegin \DIFdel{Strictly speaking}\DIFdelend \DIFaddbegin \DIFadd{Here}\DIFaddend , \DIFdelbegin \DIFdel{we should say that equation (\ref{colbeckrenner}) holds for }\DIFdelend \DIFaddbegin \DIFadd{the expressions ``}\DIFaddend almost all\DIFdelbegin \DIFdel{$\lambda$, but we need not concern ourselves here with }\DIFdelend \DIFaddbegin \DIFadd{'' is being used in }\DIFaddend the \DIFdelbegin \DIFdel{details of }\DIFdelend measure \DIFdelbegin \DIFdel{theory that would be needed to make }\DIFdelend \DIFaddbegin \DIFadd{theoretic }\DIFaddend sense\DIFdelbegin \DIFdel{of this qualification}\DIFdelend .} 
...
four criteria for a hidden variable. Since we still want to be able to make sense of PI and EA, we won't want to relax criteria \ref{hidden2} or \ref{hidden5}. That leaves the possibility of relaxing or \DIFdelbegin \DIFdel{droping }\DIFdelend \DIFaddbegin \DIFadd{dropping }\DIFaddend criteria \ref{hidden1} or \ref{hidden3}. 
...
So let $\tau_S$ be the notional mass-energy density measurement on $S$. In order to avoid undue complexity, we will assume that there is no simultaneous $\hat{T}_S$-eigenstate degeneracy so that if \DIFdelbegin \DIFdel{$\ket*{\Psi}$ and $\ket*{\Psi'}$ }\DIFdelend \DIFaddbegin \DIFadd{$\ket*{\Gamma}$ and $\ket*{\Gamma'}$ }\DIFaddend are normalized\footnote{In this section we will assume that all states are normalized.} simultaneous $\hat{T}_S$-eigenstates with simultaneous eigenvalues $\tau_S$ and $\tau_S'$ respectively, then
$$(\forall x\in S) \,(\tau_S(x)=\tau_S'(x))\implies \DIFdelbegin %DIFDELCMD < \ket*{\Psi}%%%
\DIFdelend \DIFaddbegin \ket*{\Gamma}\DIFaddend =\DIFdelbegin %DIFDELCMD < \ket*{\Psi'}%%%
\DIFdelend \DIFaddbegin \ket*{\Gamma'}\DIFaddend .\DIFaddbegin \protect\footnotemark \DIFaddend $$\DIFaddbegin \footnotetext{\DIFadd{The only reason for making this assumption is in order to avoid cluttering the notation. But if one insisted, one could add an index to all the states below in order to keep track of any degeneracy present.}}\DIFaddend This means that corresponding to $\tau_S$, there will be a unique simultaneous $\hat{T}_S$-eigenstate \DIFdelbegin \DIFdel{$\ket*{\Psi}$}\DIFdelend \DIFaddbegin \DIFadd{$\ket*{\Gamma}$}\DIFaddend , and according to the Born Rule, the notional mass-energy density measurement outcome $\tau_S$ will have been selected with a probability 
$$P(T_S = \tau_S)=|\DIFdelbegin %DIFDELCMD < \mel{\Psi}{U_{SS_0}}{\Psi_0}%%%
\DIFdelend \DIFaddbegin \mel{\Gamma}{U_{SS_0}}{\Psi_0}\DIFaddend |^2=|\DIFdelbegin %DIFDELCMD < \ip{\Psi}{\Psi_S}%%%
\DIFdelend \DIFaddbegin \ip{\Gamma}{\Psi_S}\DIFaddend |^2$$
...
possible\footnote{i.e. possible given $\ket*{\Psi_0}$ and the Born rule selection criterion. \DIFaddbegin \DIFadd{Also, recall footnote }\footreference{discreteRV} \DIFadd{that we are assuming $T_S$ is a discrete random variable so that states with zero probability are not possible.}\DIFaddend } 
...
\draw (\psione,\wttestx) node[ scoresquare, fill=gray]  {} node [black,below=6,right,scale=\textscale] {$t_m$};
	...
	\vspace*{2px}
	\DIFdelbeginFL %DIFDELCMD < \caption{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Toy model for Bell-type experiment with measurement choice]{\DIFaddendFL Depicts a Bell-type experiment where the state of some photons $\gamma_i^{(\mathcal{A}_L)}$ and $\gamma_i^{(\mathcal{A}_R)}$ on the hypersurface $S$ determines the choice of measurement parameters of the left wing and right wing of the experiment respectively, and some photons $\gamma_{m,+}^{(\mathcal{A}_L)}$ and $\gamma_{m,+}^{(\mathcal{A}_R)}$ on the hypersurface $S$ determine the measurement outcome of the experiment on the left wing and the right wing respectively. The dashed lines on the hypersurfaces $S_{n,m}$ and $S_{n,i}$ indicate other choices for the hypersurfaces, but they still lead to the same probability being calculated.   }
	\label{bellsolution}
	\end{figure}
\DIFdelbegin \DIFdel{There is a }\DIFdelend \DIFaddbegin \DIFadd{Although, this is only a one-dimensional depiction of the experimental setup, we will have to consider a toy model in more than one dimension so that it is possible to define angles between the measurement axes of the Stern-Gerlach apparatuses. In the experimental setup, there is a }\DIFaddend left wing of the experiment located in the vicinity of $z_L$, and a right wing of the experiment located in the vicinity of $z_R$. Shortly before time $t_i$, photons interact with a Stern-Gerlach apparatus on the left wing and a Stern-Gerlach apparatus on the right wing, and some of these photons eventually intersect the hypersurface $S$ so that there are subregions $S_{\mathcal{A}_L}$ and $S_{\mathcal{A}_R}$ %
...
%DIFDELCMD <   ket*{\uvbm{s}}_L$ respectively, \nomrefpage}%%%
...
In making the beables relative to a hypersurface, I am making explicit an assumption that is implicit in Schaffer's definition of completeness. The cosmos that can be duplicated by duplicating all the basic entities is duplicated at a particular time, or more generally on a particular hypersurface. It doesn't seem any more problematic to consider all the beables on a particular hypersurface than it does to consider all the beables at a particular time. But if we want to avoid monism and maximize\footnote{I will discuss below why we would want to maximize the number of beables.} the number of beables in a \DIFaddbegin \DIFadd{bounded }\DIFaddend spacelike region  $R_i$, then $R_i$ must be embedded in a hypersurface $R$ that intersects as much of $S$ as possible.\DIFaddbegin \footnote{\DIFadd{It may well turn out that there isn't a hypersurface $R$ containing $R_i$ that intersects as much of $S$ as possible. However, if we were to use the approximation scheme described by equation (\ref{tauapproxformula}), then following the argument in footnote }\footreference{finitenlimit}\DIFadd{, for each cell $c_y$ contained within $R_i$, we could find a hypersurface $S_{c_y}$ containing $c_y$ that intersected as many cells of $S$ as possible. We would then be able to find a hypersurface $R$ containing $R_i$ and $\bigcap_{c_y\subset R_i} S_{c_y}$. Although there might be several hypersurfaces that contain $R_i$ and $\bigcap_{c_y\subset R_i} S_{c_y}$, none of these hypersurfaces could contain more cells of $S$ than $R$ does while still containing $R_i$, and so these different choices of hypersurface wouldn't make any difference to the corresponding number of beables in $R_i$.}} \DIFaddend If we were just to consider the state $\ket*{\Psi_R}$ of $R$, then as Schaffer suggests, we have good reason to expect there to be only one entangled system over the whole of $R$. However, if we deem the mass-energy density on $S\cap R$ to be given by $\tau_S$, then this will correspond to applying the projection $\pi_R$ to $\ket*{\Psi_R}$, and as indicated by  the transition from equation (\ref{Psinidecomp}) to equation (\ref{piphi}), $\pi_R\ket*{\Psi_R}$ can be approximated as a product of disentangled states. The possibility that the decomposition (\ref{beablesprod}) might only be approximate rather than exact could be a slight worry, but I'm assuming that the error would be so small as not to make any discernible difference to what any intelligent beings living in the cosmos might observe.\footnote{If \DIFaddbegin \DIFadd{$\pi_R\ket*{\Psi_R}$ is only approximately a product of disentangled states, then Schaffer would probably argue that since an approximately disentangled state would still be an entangled state over the whole of $R$, the argument for monism would still hold. But as someone who finds monism philosophically unacceptable, I would say that if $\pi_R\ket*{\Psi_R}$ is to give a sensible description of reality, then (\ref{beablesprod}) would have to be exact. In order for a physical theory to ensure exactness of  (\ref{beablesprod}), }\DIFaddend we \DIFdelbegin \DIFdel{were }\DIFdelend \DIFaddbegin \DIFadd{would need }\DIFaddend to speculate further \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{about }\DIFaddend how \DIFdelbegin \DIFdel{an equality in (\ref{beablesprod}) might }\DIFdelend \DIFaddbegin \DIFadd{this exactness would }\DIFaddend come about\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{. For example }\DIFaddend one possibility might be to suppose that the universe was placed in a huge box with periodic boundary conditions. In this case, energy levels would be quantized and so it would be more plausible for  $\pi_R\ket*{\Psi_R}$ to be expressed exactly as a product of disentangled states. \DIFaddbegin \DIFadd{Another approach that might also lead to the exactness of  (\ref{beablesprod}) is quantum loop gravity where space is quantized -- see \mbox{%DIFAUXCMD
\cite[9--16]{RovelliCarlo2014CLQG}}\hskip0pt%DIFAUXCMD
.}\DIFaddend } 
...
But with the Aristotelian-inspired notion of time I've been discussing, the question of what happens after $S$ doesn't arise, for the whole of time is understood in terms of the actualization of $S$. With this understanding of time, we can speak of the state $\ket*{\Gamma_{\tau_S}}$ being at the end of time insofar as there is no other state $\ket*{\gamma}$  on a region of $S$ with $\ket*{\gamma}>\ket*{\Gamma_{\tau_S}}$ -- the state $\ket*{\Gamma_{\tau_S}}$ is like the completed tapestry, so to speak. \DIFaddbegin \DIFadd{Theologians may find it interesting to speculate on whether the idea that there is a state $\ket*{\Gamma_{\tau_S}}$ at the end of time bears any relationship to the theological notion of the }\emph{\DIFadd{eschaton}}\index{eschaton}\DIFadd{,}\footnote{\DIFadd{This comes from the Greek word \textgreek{ἔσχατος}   meaning last or extreme. }} \DIFadd{a term that has been used by theologians such as C. H. Dodd to mean the divinely ordained climax of history.}\footnote{\DIFadd{See \mbox{%DIFAUXCMD
\cite[36]{DoddC.H1961Tpot}}\hskip0pt%DIFAUXCMD
