...
$$(\forall x\in S) \,(\tau_S(x)=\tau_S'(x))\implies \ket*{\Psi}=\ket*{\Psi'}.\protect\footnotemark $$\footnotetext{The only reason for making this assumption is in order to avoid cluterring the notation. But if one insisted, one could add an index to all the states below in order to keep track of any degeneracy present.}This means that corresponding to $\tau_S$, there will be a unique simultaneous $\hat{T}_S$-eigenstate $\ket*{\Psi}$, and according to the Born Rule, the notional mass-energy density measurement outcome $\tau_S$ will have been selected with a probability 
