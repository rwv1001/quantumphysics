...
%DIF DEL revised010823.tex   Sat Aug  5 23:33:46 2023
%DIF ADD revised030923.tex   Sun Sep  3 23:54:49 2023
...
Kent implicitly assumes that a specification of the stress-energy tensor $T^{\mu\nu}(y)$ for all spacetime locations $y$ between $S_0$ and $S$ will be sufficient to give a macroscopic description of physical reality between $S_0$ and $S$.\footnote{See \cite[2]{Kent2014} where he talks about giving a description of reality between $S_0$ and $S$.} This seems like a reasonable assumption, since from the $T^{00}(y)$ component of the stress-energy tensor, we will be able to tell how much energy is present in the vicinity of $y$, and from the other components of the stress-energy tensor, we will be able to work out how much of this energy is due to mass and how much is due to motion.\footnote{\DIFdelbegin \DIFdel{Note that it }\DIFdelend \DIFaddbegin \DIFadd{It }\DIFaddend doesn't matter that \DIFaddbegin \DIFadd{the  }\DIFaddend $\hat{T}^{\mu\nu}(y)$ won't typically commute for all $\mu$ and $\nu$ \DIFdelbegin \DIFdel{. This is }\DIFdelend because the value of \DIFdelbegin \DIFdel{$T^{\mu\nu}$ }\DIFdelend \DIFaddbegin \DIFadd{$T^{\mu\nu}(y)$ }\DIFaddend that Kent ascribes to the spacetime location $y$ is not the eigenvalue \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend some simultaneous eigenstate of the $\hat{T}^{\mu\nu}(y)$ for all $\mu$ and $\nu$ (\DIFdelbegin \DIFdel{non-communitativity }\DIFdelend \DIFaddbegin \DIFadd{non-commutativity }\DIFaddend of the $\hat{T}^{\mu\nu}(y)$ would guarantee that there is no such simultaneous eigenstate). Rather, the value of \DIFdelbegin \DIFdel{$T^{\mu\nu}$ }\DIFdelend \DIFaddbegin \DIFadd{$T^{\mu\nu}(y)$ }\DIFaddend that Kent ascribes to the spacetime location $y$ is an expectation value of  $\hat{T}^{\mu\nu}(y)$ conditioned on the mass-energy density $T_S(x)$ on $S$, and so the non-commutativity of the $\hat{T}^{\mu\nu}(y)$ is irrelevant when calculating this expectation value  \DIFaddbegin \DIFadd{for all $\mu$ and $\nu$}\DIFaddend . \DIFdelbegin \DIFdel{One }\DIFdelend \DIFaddbegin \DIFadd{Nevertheless, one }\DIFaddend might still wonder whether it \DIFaddbegin \DIFadd{is }\DIFaddend sensible to ascribe the   \DIFdelbegin \DIFdel{the conditioned }\DIFdelend expectation value of  $\hat{T}^{\mu\nu}(y)$ \DIFaddbegin \DIFadd{conditioned on the mass-energy density $T_S(x)$ on $S$ }\DIFaddend to \DIFdelbegin \DIFdel{$T^{\mu\nu}$}\DIFdelend \DIFaddbegin \DIFadd{$T^{\mu\nu}(y)$}\DIFaddend . But as we will see in section \ref{KentconsistentQT}, conditioning \DIFaddbegin \DIFadd{the expectation of $\hat{T}^{\mu\nu}(y)$ }\DIFaddend on the mass-energy density $T_S(x)$ on $S$ \DIFdelbegin \DIFdel{will allow us }\DIFdelend \DIFaddbegin \DIFadd{is equivalent }\DIFaddend to \DIFdelbegin \DIFdel{obtain a conitioned }\DIFdelend \DIFaddbegin \DIFadd{taking the expectation $\ev*{ \hat{T}^{\mu\nu}(y)}{\Gamma}$ defined in the usual way, where the conditioning is now encapsulated in $\ket*{\Gamma}$ (which I refer to as the conditioned }\DIFaddend quantum state\DIFaddbegin \DIFadd{). Moreover}\DIFaddend , \DIFdelbegin \DIFdel{and }\DIFdelend we should \DIFdelbegin \DIFdel{be able to }\DIFdelend \DIFaddbegin \DIFadd{typically }\DIFaddend expect there to be sufficient information in the mass-energy density $T_S(x)$ on $S$ to ensure that \DIFdelbegin \DIFdel{the conditioned quantum state }\DIFdelend \DIFaddbegin \DIFadd{$\ket*{\Gamma}$ is very }\DIFaddend close \DIFdelbegin \DIFdel{enough }\DIFdelend to being an eigenstate of all the \DIFdelbegin \DIFdel{$\hat{T}^{\mu\nu}(y)$ }\DIFdelend \DIFaddbegin \DIFadd{$\hat{T}^{\mu\nu}(c_y)$ where $\hat{T}^{\mu\nu}(c_y)$ is the averaged stress-energy operator over some mesoscopic three-dimensional spatial cell $c_y$ containing the spacetime location $y$ (i.e.  $\hat{T}^{\mu\nu}(c_y)=\frac{1}{|c_y|}\int_{y'\in c_y} \hat{T}^{\mu\nu}(y')\dd y'$ where $|c_y|$ is the volume of the  three-dimensional spatial cell $c_y$). This means there will be numerical quantities $t^{\mu\nu}$ for all $\mu$ and $\nu$ such that $\hat{T}^{\mu\nu}(c_y)\ket*{\Gamma}\approx t^{\mu\nu}\ket*{\Gamma}$ }\DIFaddend to \DIFaddbegin \DIFadd{very good approximation,  and the conditioned expectation value of $\hat{T}^{\mu\nu}(y)$ will thus be $\ev*{ \hat{T}^{\mu\nu}(c_y)}{\Gamma}\approx t^{\mu\nu}.$   The $t^{\mu\nu}$-values that approximate the  conditioned expectation value of $\hat{T}^{\mu\nu}(c_y)$ will then }\DIFaddend form a  \DIFdelbegin \DIFdel{macroscopic picture }\DIFdelend \DIFaddbegin \DIFadd{good approximation }\DIFaddend of \DIFdelbegin \DIFdel{stess-energy }\DIFdelend \DIFaddbegin \DIFadd{the average stress-energy }\DIFaddend tensor in the vicinity of $y$ \DIFaddbegin \DIFadd{at the macroscopic level}\DIFaddend .} Thus, from the stress-energy tensor, we will be able to form a picture of where things are and whether things are in equilibrium or whether there are flows of matter and energy from one region to another. Such a description of reality will also include information about measurement readings that scientists observe when performing their experiments, so although  we wouldn't expect there to be a complete specification of physical reality down to the microscopic level in terms of the stress-energy tensor, there will be enough information in the stress-energy tensor at the macroscopic level to make various scientific claims about physical reality at the microscopic level.  
